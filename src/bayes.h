...
#define YFLT_MAX DBL_MAX  /* maximum possible value that can be stored in YFlt */
#define YFLT_MIN DBL_MIN  /* minimum possible value that can be stored in YFlt */
#define YFLT_NEG_MAX (-DBL_MAX)  /* maximum possible negative value that can be stored in YFlt */
typedef double YFlt;      /* double used for parameter values and generally for floating point values, 
...
/* Molecular model constants — retained for dead code in model.c/likelihood.c
   that references them. Will be removed when Tier 2/3 cleanup is complete. */
...
#define MAX_NUM_USERTYPES       50
#define MAX_USERTYPE_NAME       100

typedef struct usertype
    {
    char        name[MAX_USERTYPE_NAME];  /* name of the usertype                      */
    int         nStates;                  /* number of states                          */
    YFlt      *rates;                   /* off-diagonal rates (nStates * nStates)    */
    } UserType;
...
/*paramType — molecular params (P_TRATIO, P_REVMAT, P_OMEGA, P_AAMODEL) removed */
...
#define P_SPECRATE              8
#define P_EXTRATE               9
#define P_FOSLRATE              10
#define P_POPSIZE               11
#define P_BMCORR                12
#define P_BMSIGMA               13
#define P_GROWTH                14
#define P_CPPMULTDEV            15
#define P_CPPRATE               16
#define P_CPPEVENTS             17
#define P_TK02VAR               18
#define P_TK02BRANCHRATES       19
#define P_IGRVAR                20
#define P_IGRBRANCHRATES        21
#define P_ILNVAR                22
#define P_ILNBRANCHRATES        23
#define P_CLOCKRATE             24
#define P_SPECIESTREE           25
#define P_GENETREERATE          26
#define P_MIXTURE_RATES         27
#define P_MIXEDVAR              28
#define P_MIXEDBRCHRATES        29
#define P_WNVAR                 30
#define P_WNBRANCHRATES         31
...
#define MAX_NUM_USERTREES       200     /* maximum number of user trees yvyra will read */
...
typedef YFlt (*LnPriorProbFxn)(YFlt val, YFlt *priorParams);

/* typedef for a ln prior prob ratio fxn */
typedef YFlt (*LnPriorRatioFxn)(YFlt newVal, YFlt oldVal, YFlt *priorParams);

typedef struct
    {
    YFlt          sum;            /* sum of standard deviations */
    YFlt          max;            /* maximum standard deviation */
    YFlt          numPartitions;
    YFlt          numSamples;
    YFlt          avgStdDev;
    YFlt          **pair;
...
typedef struct node
    ...
    YFlt          d;                      /*!< scratch variable                           */
    BitsLong        *partition;             /*!< pointer to bitfield describing splits      */
    YFlt          length;                 /*!< length of pending branch                   */
    YFlt          nodeDepth;              /*!< node depth (height)                        */
    YFlt          age;                    /*!< age of node                                */
...
typedef struct pNode
    ...
    YFlt          length;             /*!< age of node                                  */
    YFlt          depth;              /*!< depth (height) of node                       */
    YFlt          age;                /*!< age of node                                  */
    YFlt          support, f;         /*!< scratch variables                            */
...
typedef struct 
    ...
    YFlt          ***position;         /*!< position of branch events                   */
    YFlt          ***rateMult;         /*!< parameter of branch events                  */
    YFlt          **effectiveBrLen;    /*!< effective branch lengths of ebl set         */
    int             brlensDef;           /*!< are brlens defined ?                        */
    int             isRooted;            /*!< is tree rooted?                             */
    int             isClock;             /*!< is tree clock?                              */
    int             isCalibrated;        /*!< is tree calibrated?                         */
    int             isRelaxed;           /*!< is tree relaxed?                            */
    YFlt          clockRate;           /*!< clock rate                                  */
    int             popSizeSet;          /*!< does tree have a population size set?       */
    YFlt          *popSize;            /*!< the population size                         */
...
typedef struct param
    ...
    YFlt          *values;            /* main values of parameter                       */
    YFlt          *subValues;         /* subvalues of parameter                         */
    ...
    YFlt          min;                /* minimum value of parameter                     */
    YFlt          max;                /* maximum value of parameter                     */
    ...
    int             *sympiUserTypeIndex;/* pointer to usertype index (std chars)          */
    ...
    YFlt          *stdStateFreqs;     /* pointer to std state frequencies               */
    int             **nEvents;          /* number of branch events for Cpp model          */
                                        /* nEvents[0..2*numCains][0..numNodes=2*numTaxa]  */
    YFlt          ***position;        /* event positions for Cpp relaxed clock model    */
    YFlt          ***rateMult;        /* rate multipliers for Cpp relaxed clock model   */
    int             affectsLikelihood;  /* does parameter directly influence likelihood?  */
    YFlt*         priorParams;        /* pointer to the prior parameters                */
...
typedef int (MoveFxn)(Param *param, int chain, RandLong *seed, YFlt *lnPriorRatio, YFlt *lnProposalRatio, YFlt *mvp);

/* typedef for an ApplicFxn */
typedef int (ApplicFxn)(Param *param);

/* typedef for an AutotuneFxn */
typedef void (AutotuneFxn)(YFlt acceptanceRate, YFlt targetRate, int batch, YFlt *tuningParameter, YFlt minTuning, YFlt maxTuning);
...
typedef struct
    ...
    YFlt       relProposalProb;    /* default relative proposal probability        */
    int          numTuningParams;    /* number of tuning parameters                  */
    YFlt       tuningParam[5];     /* default tuning parameters for the proposal   */
    YFlt       minimum[5];         /* minimum values for tuning params             */
    YFlt       maximum[5];         /* maximum values for tuning params             */
    ...
    YFlt       targetRate;         /* default target acceptance rate for autotuning */
    } MoveType;
...
typedef struct
    ...
    YFlt      *relProposalProb;   /* the relative proposal probability            */
    YFlt      *cumProposalProb;   /* the cumulative proposal probability          */
    ...
    YFlt      *targetRate;        /* target acceptance rate for autotuning        */
    YFlt      *lastAcceptanceRate;/* acceptance rate in last complete batch       */
    YFlt      **tuningParam;      /* tuning parameters for the move               */
    } MCMCMove;
...
typedef int (*LikeFxn)(TreeNode *, int, int, YFlt *, int);
...
typedef struct model
    ...
    YFlt      brownScaleFix;
    YFlt      brownScaleUni[2];
    YFlt      brownScaleGamma[2];
    ...
    YFlt      speciationFix;
    YFlt      speciationUni[2];
    YFlt      speciationExp;
    char        extinctionPr[100];     /* prior on relative extinction rate (turnover)      */
    YFlt      extinctionFix;
    YFlt      extinctionBeta[2];
    YFlt      extinctionExp;
    char        fossilizationPr[100];  /* prior on fossilization rate (sampling proportion) */
    YFlt      fossilizationFix;
    YFlt      fossilizationBeta[2];
    YFlt      fossilizationExp;
    char        sampleStrat[100];      /* taxon sampling strategy (for b-d process)         */
    int         birthRateShiftNum;     /* number of birth rate shifts                       */
    YFlt      birthRateShiftTime[100];    /* birth rate shifting times                    */
    int         deathRateShiftNum;     /* number of death rate shifts                       */
    YFlt      deathRateShiftTime[100];    /* death rate shifting times                    */
    int         fossilSamplingNum;     /* number of fossil sampling rate shiftings / slice sampling events */
    YFlt      fossilSamplingTime[100];    /* fossil sampling rate shifting times          */
    // YFlt     *fossilSamplingProb; /* fossil slice sampling probs (rho_i)               */
    YFlt      sampleProb;            /* extant taxon sampling fraction (rho)              */
    ...
    YFlt      clockRateNormal[2];
    YFlt      clockRateLognormal[2];
    YFlt      clockRateGamma[2];
    YFlt      clockRateExp;
    YFlt      clockRateFix;
    char        popSizePr[100];        /* prior on population size                      */
    YFlt      popSizeFix;
    YFlt      popSizeUni[2];
    YFlt      popSizeLognormal[2];
    YFlt      popSizeGamma[2];
    YFlt      popSizeNormal[2];
    ...
    char        inferSiteLikes[5];     /* should site lnLs be output (Yes/No)?          */
...
typedef struct chain
    ...
    YFlt      alphaSS;               /* Beta values are distributed according to quantiles of Beta(alphaSS,1.0) distribution */
    int         backupCheckSS;         /* Frequency of checkpoints backup               */
    YFlt      chainTemp;             /* chain temperature                             */
    int         userDefinedTemps;      /* should we use the users temperatures?         */
    YFlt      userTemps[MAX_CHAINS]; /* user-defined chain temperatures               */
    ...
    YFlt      weightScheme[3];       /* percent chars to increase/decrease in weight  */
    ...
    YFlt      burninFraction;        /* the sample fraction to discard as burnin      */
    int         allComps;              /* top conv diagnosis for all pairs?             */
    YFlt      minPartFreq;           /* minimum partition frequency for conv diagn    */
    YFlt      stopVal;               /* top conv diagn value to reach before stopping */
...
typedef struct modelinfo
    ...
    int         numCorrGroups;              /* number of ascertainment correction groups */
    int         corrGroupNStates[50];       /* nStates per correction group             */
    int         corrGroupCType[50];         /* cType per correction group               */
    int         corrGroupUserTypeIdx[50];   /* userTypeIdx per group (-1 if not USERTYPE)*/
    int         corrGroupDummyStart[50];    /* first dummy char index for each group    */
    ...
    int         *cType;                     /* whether char is ord, unord, irrev, or usertype */
    int         *userTypeIdx;               /* usertype index per char (-1 if none)         */
    int         hasUserType;                /* does division have usertype chars? (YES/NO)  */
    int         hasTipWeights;              /* does division have confidence-weighted tips?  */
    ...
    YFlt      parsTreeLength[MAX_CHAINS*2];   /* parsimony tree lengths for chains        */
    ...
    YFlt      lnLike[MAX_CHAINS];         /* log like for chain                           */
    ...
    int         printSiteLikes;             /* should site lnLs be printed (YES/NO)         */
    YFlt      *siteLnL;                   /* per-site log-likelihoods (numChars)           */
...
typedef struct sumt
    ...
    YFlt      minPartFreq;           /* minimum part. freq. for overall diagnostics   */
    ...
    YFlt      minBrParamFreq;        /* threshold for printing branch params to file  */
    } Sumt;
...
typedef struct comptree
    ...
    YFlt      minPartFreq;           /* use partitions with frequency >= minPartFreq  */
    } Comptree;
...
typedef struct sump
    ...
    int         HPD;                   /* use highest posterior density?                */
    YFlt      minProb;               /* cut-off for model probabilities to show       */
    } Sump;
...
typedef struct sumss
    ...
    YFlt      discardFraction;       /* Proportion of samples discarded when plotting step plot.*/
    } Sumss;
...
typedef struct 
    ...
    int         userTypeIndex;         /* index into userType array (-1 if none)        */
    ...
    YFlt      weight;                /* character weight (default 1.0)                */
    }
    CharInformation;
...
extern UserType         userTypes[MAX_NUM_USERTYPES];           /* user-defined rate matrices                   */
extern int              numUserTypes;                           /* number of defined user types                 */
...
extern YFlt           *tipWeights;                            /* confidence weights for tips (NULL if none)    */
extern char             **charLabels;                           /* character names (NULL if not set)             */
extern char             ***charStateNames;                      /* per-char state names [nChar][nStates] or NULL */
extern int              *charNStateNames;                       /* per-char count of named states (NULL if none) */
extern int              hasTipWeights;                          /* flag: any confidence-weighted tips present?   */
...
extern int              inferSiteLikes;                         /* should site lnLs be output (y/n)              */
...
extern int              inYvyraBlock;                         /* flag for whether we are in a yvyra block      */
...
extern YFlt           *paramValues;                           /* values of parameters                          */
...
extern YFlt           *parameterValues;                       /* vector holding sump or plot parameters        */
...
extern YFlt           *stdStateFreqs;                         /* std char state frequencies                    */
...
extern YFlt           *tempNum;                               /* vector of numbers used for setting arrays     */
...
extern YFlt           myStateInfo[7];                         /* likelihood/prior/heat/ran/moveInfo vals of me              */
extern YFlt           partnerStateInfo[7];                    /* likelihood/prior/heat/ran/moveInfo vals of partner         */
#endif
