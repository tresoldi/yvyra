...
int Move_AddDeleteCPPEvent (Param *param, int chain, RandLong *seed, YFlt *lnPriorRatio, YFlt *lnProposalRatio, YFlt *mvp)
{
    ...
    YFlt      sigma, m, lognormalLnProb, **position, **rateMultiplier, length, pos, rate;
    TreeNode    *p, *q;
    ModelInfo   *model;
    Tree        *t;
    ...
    if (addEvent == NO)
        ...
        if (numEvents-1 > 0)
            {
            position[p->index] = (YFlt *) SafeRealloc ((void *) position[p->index], (numEvents-1)*sizeof(YFlt));
            rateMultiplier[p->index] = (YFlt *) SafeRealloc ((void *) rateMultiplier[p->index], (numEvents-1)*sizeof(YFlt));
...
int Move_Beta (Param *param, int chain, RandLong *seed, YFlt *lnPriorRatio, YFlt *lnProposalRatio, YFlt *mvp)
{
    ...
    YFlt      oldB, newB, minB, maxB, *bs, ran, factor, tuning,
                x, y;
    ModelParams *mp;
    ...
    *lnProposalRatio = log (newB / oldB);
    ...
    if (isPriorExp == YES)
        {
        *lnPriorRatio = (mp->symBetaExp) * (oldB - newB);
        }
    else
        *lnPriorRatio = 0.0;
...
int Move_BrLen (Param *param, int chain, RandLong *seed, YFlt *lnPriorRatio, YFlt *lnProposalRatio, YFlt *mvp)
{
    ...
    if (param->paramId == BRLENS_EXP)
        (*lnPriorRatio) = (mp->brlensExp) * (m - newM);
    /* Dirichlet or twoExp prior */
...
int Move_BMcorr (Param *param, int chain, RandLong *seed, YFlt *lnPriorRatio, YFlt *lnProposalRatio, YFlt *mvp)
{
    ...
    int             i, isValidP;
    YFlt          oldP, newP, window, minP, maxP;
    ModelParams     *mp;
...
int Move_BMsigma (Param *param, int chain, RandLong *seed, YFlt *lnPriorRatio, YFlt *lnProposalRatio, YFlt *mvp)
{
    ...
    int         i, isValidL;
    YFlt      oldL, newL, minL, maxL, tuning;
    ModelParams *mp;
...
int Move_ClockRate_M (Param *param, int chain, RandLong *seed, YFlt *lnPriorRatio, YFlt *lnProposalRatio, YFlt *mvp)
{
    ...
    int             i, j, k, *nEvents;
    YFlt          oldRate, newRate, factor, lambda, nu, var, *brlens, *rate,
    ...
    for (i = 0; i < numTrees; i++)
        ...
        if (!strcmp(mp->clockPr,"Coalescence"))
            ...
            if (LnCoalescencePriorPr (oldT, &oldLnPrior, oldTheta, growth) == ERROR)
                {
                YvyraPrint ("%s   Problem calculating prior for coalescent process\n", spacer);
                return (ERROR);
                }
            ...
            if (subParm->paramType == P_CPPEVENTS)
                ...
                for (j = 0; j < t->nNodes-2; j++)
                    ...
                    if (p->length > 0.0)  // not ancestral fossil
                        {
                        (*lnPriorRatio) -= LnProbLogNormal_Mean_LogVar (rate[q->anc->index], nu*q->length, rate[q->index]);
                        (*lnPriorRatio) += LnProbLogNormal_Mean_LogVar (rate[p->anc->index], nu*p->length, rate[p->index]);
                ...
                for (j = 0; j < t->nNodes-2; j++)
                    ...
                    if (p->length > 0.0)  // not ancestral fossil
                        {
                        (*lnPriorRatio) -= LnProbLogNormal_Mean_Var (1.0, nu, rate[q->index]);
                        (*lnPriorRatio) += LnProbLogNormal_Mean_Var (1.0, nu, rate[p->index]);
...
int Move_CPPEventPosition (Param *param, int chain, RandLong *seed, YFlt *lnPriorRatio, YFlt *lnProposalRatio, YFlt *mvp)
{
    ...
    int         i, j, k, *nEvents;
    YFlt      pos, temp, **position, **rateMultiplier;
    TreeNode    *p=NULL, *q;
    Tree        *t;
...
int Move_CPPRate (Param *param, int chain, RandLong *seed, YFlt *lnPriorRatio, YFlt *lnProposalRatio, YFlt *mvp)
{
    ...
    YFlt      oldLambda, newLambda, treeLength, tuning;
    Model       *mp;
    TreeNode    *p;
    Tree        *t;
...
int Move_CPPRateMultiplier_M (Param *param, int chain, RandLong *seed, YFlt *lnPriorRatio, YFlt *lnProposalRatio, YFlt *mvp)
{
    ...
    int         i, j, k, *nEvents;
    YFlt      newRateMultiplier, oldRateMultiplier, tuning, minM, maxM, sigma, **rateMultiplier;
    TreeNode    *p = NULL;
    ModelInfo   *m;
    Tree        *t;
    TreeNode    *q;
...
int Move_CPPRateMultiplierRnd (Param *param, int chain, RandLong *seed, YFlt *lnPriorRatio, YFlt *lnProposalRatio, YFlt *mvp)
{
    ...
    int         i, j, k, *nEvents;
    YFlt      sigma, newRateMultiplier, oldRateMultiplier, **rateMultiplier;
    TreeNode    *p=NULL, *q;
    ModelInfo   *m;
    Tree        *t;
...
int Move_AddBranch (Param *param, int chain, RandLong *seed, YFlt *lnPriorRatio, YFlt *lnProposalRatio, YFlt *mvp)
{
    ...
    YFlt minDepth, maxDepth, newLength, clockRate, x, oldQLength, oldRLength,
    ...
    for (i=0; i<param->nSubParams; i++)
        ...
        if (subParm->paramType == P_CPPEVENTS)
            ...
            YvyraPrint ("%s   CPP clock is not compatible with Fossilization prior currently\n", spacer);
            return (ERROR);
            }
        else if ( subParm->paramType == P_TK02BRANCHRATES)
            {
            ...
            (*lnPriorRatio) -= LnProbLogNormal_Mean_LogVar (rate[q->index], nu*oldRLength, rate[r->index]);
            (*lnPriorRatio) += LnProbLogNormal_Mean_LogVar (rate[q->index], nu* p->length, rate[p->index]);
            (*lnPriorRatio) += LnProbLogNormal_Mean_LogVar (rate[q->index], nu* r->length, rate[r->index]);
            if (q->anc->anc != NULL)
                {
                (*lnPriorRatio) -= LnProbLogNormal_Mean_LogVar (rate[q->anc->index], nu*oldQLength, rate[q->index]);
                (*lnPriorRatio) += LnProbLogNormal_Mean_LogVar (rate[q->anc->index], nu* q->length, rate[q->index]);
                }
...
int Move_DelBranch (Param *param, int chain, RandLong *seed, YFlt *lnPriorRatio, YFlt *lnProposalRatio, YFlt *mvp)
{
    ...
    int    i, j, k, mFossil, kFossil;
    YFlt minDepth, maxDepth, clockRate, x, oldPLength, oldQLength, oldRLength,
           *brlens=NULL, nu=0.0, var=0.0, *rate=NULL;
    TreeNode    *p=NULL, *q=NULL, *r;
    Tree        *t;
    ModelParams *mp;
    ...
    for (i=0; i<param->nSubParams; i++)
        ...
        if (subParm->paramType == P_CPPEVENTS)
        ...
        else if ( subParm->paramType == P_TK02BRANCHRATES)
           {
            ...
            /* prior ratio */
            (*lnPriorRatio) -= LnProbLogNormal_Mean_LogVar (rate[q->index], nu*oldPLength, rate[p->index]);
            (*lnPriorRatio) -= LnProbLogNormal_Mean_LogVar (rate[q->index], nu*oldRLength, rate[r->index]);
            (*lnPriorRatio) += LnProbLogNormal_Mean_LogVar (rate[q->index], nu* r->length, rate[r->index]);
            if (q->anc->anc != NULL)
                {
...
int Move_Extinction (Param *param, int chain, RandLong *seed, YFlt *lnPriorRatio, YFlt *lnProposalRatio, YFlt *mvp)
{
    ...
    YFlt      *valPtr, oldM, newM, window, minM, maxM, *sR, *eR, sF, *fR, oldLnPrior, newLnPrior,
    ...
    *lnProposalRatio = 0.0;
    ...
    if (!strcmp(mp->clockPr,"Birthdeath"))
        ...
        *valPtr = newM;  // update with new value
        ...
        YvyraPrint ("%s   Move_Extinction not applicable\n", spacer);
        return (ERROR);
        }
    ...
    else if (param->paramId == EXTRATE_EXP)
        *lnPriorRatio = newLnPrior - oldLnPrior - (mp->extinctionExp)*(newM - oldM);
...
int Move_Fossilization (Param *param, int chain, RandLong *seed, YFlt *lnPriorRatio, YFlt *lnProposalRatio, YFlt *mvp)
{
    ...
    /* get minimum and maximum values */
    if (param->paramId == FOSLRATE_BETA)
        {
        minM = 0.000001;
        maxM = 1.0;
        }
    ...
    *valPtr = newM;  // update with new value
    ...
    else if (param->paramId == FOSLRATE_EXP)
        *lnPriorRatio = newLnPrior - oldLnPrior - (mp->fossilizationExp)*(newM - oldM);
...
int Move_ExtSPR (Param *param, int chain, RandLong *seed, YFlt *lnPriorRatio, YFlt *lnProposalRatio, YFlt *mvp)
{
    ...
    YFlt      m, x, y, tuning, maxV, minV, extensionProb;
...
int Move_ExtSPRClock (Param *param, int chain, RandLong *seed, YFlt *lnPriorRatio, YFlt *lnProposalRatio, YFlt *mvp)
{
    ...
    YFlt      x, y=0.0, oldBrlen=0.0, newBrlen=0.0, extensionProb, nu, var, *rate=NULL,
...
#   endif
    ...
    for (i=0; i<param->subParams[0]->nSubParams; i++)
        ...
        if (subParm->paramType == P_CPPEVENTS)
            ...
            if (n2 > 0)
                {
                position[a->index] = (YFlt *) SafeRealloc ((void *) position[a->index], (n1+n2) * sizeof (YFlt));
                rateMultiplier[a->index] = (YFlt *) SafeRealloc ((void *) rateMultiplier[a->index], (n1+n2) * sizeof (YFlt));
                }
            for (j=0; j<n1; j++)
            ...
            rate = GetParamVals (subParm, chain, state[chain]);
            if (v->length > 0.0)
                (*lnPriorRatio) -= LnProbLogNormal_Mean_LogVar(rate[v->anc->index], nu*v->length, rate[v->index]);
            (*lnPriorRatio) -= LnProbLogNormal_Mean_LogVar(rate[a->anc->index], nu*a->length, rate[a->index]);
            (*lnPriorRatio) -= LnProbLogNormal_Mean_LogVar(rate[u->anc->index], nu*u->length, rate[u->index]);
            (*lnPriorRatio) += LnProbLogNormal_Mean_LogVar(rate[u->anc->index], nu*(a->length+u->length), rate[a->index]);
    ...
    for (i=0; i<param->subParams[0]->nSubParams; i++)
        ...
        if (subParm->paramType == P_CPPEVENTS)
            ...
            if (n5 > 0)
                {
                position[u->index] = (YFlt *) SafeRealloc ((void *) position[u->index], n5 * sizeof (YFlt));
                rateMultiplier[u->index] = (YFlt *) SafeRealloc ((void *) rateMultiplier[u->index], n5 * sizeof (YFlt));            
                ...
                if (n4 > 0)
                    {
                    position[a->index] = (YFlt *) SafeRealloc ((void *) position[a->index], n4 * sizeof (YFlt));
                    rateMultiplier[a->index] = (YFlt *) SafeRealloc ((void *) rateMultiplier[a->index], n4 * sizeof (YFlt));
                    for (j=0; j<n4; j++)
...
int Move_ExtSS (Param *param, int chain, RandLong *seed, YFlt *lnPriorRatio, YFlt *lnProposalRatio, YFlt *mvp)
{
    ...
    YFlt      m, x, tuning, maxV, minV, extensionProb;
...
int Move_ExtSSClock (Param *param, int chain, RandLong *seed, YFlt *lnPriorRatio, YFlt *lnProposalRatio, YFlt *mvp)
{
    ...
    YFlt      x, oldALength, oldCLength, extensionProb, nu, var, *rate,
    ...
    for (i=0; i<param->subParams[0]->nSubParams; i++)
        ...
        if (subParm->paramType == P_CPPEVENTS)
            ...
            (*lnPriorRatio) -= LnProbLogNormal_Mean_LogVar (rate[c->anc->index], nu*oldALength, rate[a->index]);
            (*lnPriorRatio) += LnProbLogNormal_Mean_LogVar (rate[a->anc->index], nu* a->length, rate[a->index]);
            (*lnPriorRatio) -= LnProbLogNormal_Mean_LogVar (rate[a->anc->index], nu*oldCLength, rate[c->index]);
            (*lnPriorRatio) += LnProbLogNormal_Mean_LogVar (rate[c->anc->index], nu* c->length, rate[c->index]);
            brlens[a->index] = a->length * (rate[a->index] + rate[a->anc->index])/2.0;
...
#   endif
    ...
    if (isVPriorExp == YES)
        (*lnPriorRatio) += (mp->brlensExp) * (m - v->length);
    ...
    if (t->isRooted == NO || u->anc->anc != NULL) 
        ...
        if (isVPriorExp == YES)
            (*lnPriorRatio) += (mp->brlensExp) * (m - u->length);
        }
    u->upDateTi = YES;
...
int Move_GeneRate_Dir (Param *param, int chain, RandLong *seed, YFlt *lnPriorRatio, YFlt *lnProposalRatio, YFlt *mvp)
{
    ...
    YFlt      alphaPi, *value, *subValue, numSites, *alphaDir, x, y, sum,
                rate_pot, *dirParm, *oldRate, *newRate;

    /* allocate memory */
    dirParm = (YFlt *) SafeCalloc (3*(numTopologies-1), sizeof(YFlt));
...
{
    YFlt          oldG, newG, lambda, minG, maxG, ran, oldLnPrior, newLnPrior, curTheta;
    ModelParams     *mp;
    ModelInfo       *m;
    Tree            *t;
...
int Move_IgrBranchRate (Param *param, int chain, RandLong *seed, YFlt *lnPriorRatio, YFlt *lnProposalRatio, YFlt *mvp)
{
    ...
    int         i;
    YFlt      newRate, oldRate, tuning, minR, maxR, igrvar, *igrRate, *brlens;
    TreeNode    *p = NULL;
    ModelInfo   *m;
    Tree        *t;
    TreeNode    *q;
...
int Move_IgrVar (Param *param, int chain, RandLong *seed, YFlt *lnPriorRatio, YFlt *lnProposalRatio, YFlt *mvp)
{
    ...
    int         i, j;
    YFlt      oldVar, newVar, minVar, maxVar, tuning, *rate;
    Model       *mp;
    TreeNode    *p;
    Tree        *t;
...
int Move_IlnBranchRate (Param *param, int chain, RandLong *seed, YFlt *lnPriorRatio, YFlt *lnProposalRatio, YFlt *mvp)
{
    ...
    int         i;
    YFlt      newRate, oldRate, tuning, minR, maxR, ilnvar, *ilnRate, *brlens;
    TreeNode    *p = NULL;
    ModelInfo   *m;
    Tree        *t;
    TreeNode    *q;
    ...
    (*lnPriorRatio) = LnProbLogNormal_Mean_Var (1.0, ilnvar, newRate)
                    - LnProbLogNormal_Mean_Var (1.0, ilnvar, oldRate);
...
int Move_IlnVar (Param *param, int chain, RandLong *seed, YFlt *lnPriorRatio, YFlt *lnProposalRatio, YFlt *mvp)
{
    ...
    int         i, j;
    YFlt      oldVar, newVar, minVar, maxVar, tuning, *ilnRate;
    Model       *mp;
    TreeNode    *p;
    Tree        *t;
    ...
    for (i=0; i<param->nSubParams; i++)
        ...
        for (j=0; j<t->nNodes-2; j++)
            ...
            if (p->length > 0.0)  // not ancestral fossil
                {
                (*lnPriorRatio) -= LnProbLogNormal_Mean_Var (1.0, oldVar, ilnRate[p->index]);
                (*lnPriorRatio) += LnProbLogNormal_Mean_Var (1.0, newVar, ilnRate[p->index]);
                }
            }
        }
...
int Move_MixedBranchRate (Param *param, int chain, RandLong *seed, YFlt *lnPriorRatio, YFlt *lnProposalRatio, YFlt *mvp)
{
    ...
    int         i, *rclModel=NULL;
    YFlt      newRate, oldRate, tuning, minR, maxR, mxvar, *mxRate, *brlens;
    TreeNode    *p = NULL, *q;
    ModelInfo   *m;
    Tree        *t;
    ...
    if (*rclModel == RCL_IGR)
        ...
        {
        (*lnPriorRatio) -= LnProbLogNormal_Mean_Var (1.0, mxvar, oldRate);
        (*lnPriorRatio) += LnProbLogNormal_Mean_Var (1.0, mxvar, newRate);
        }
        ...
        for (j=0; j<t->nNodes-2; j++)
            ...
            if (p->length > 0.0) { // not ancestral fossil
                if (*rclModel == RCL_IGR)
                    ...
                    {
                    (*lnPriorRatio) -= LnProbLogNormal_Mean_Var (1.0, oldVar, rate[p->index]);
                    (*lnPriorRatio) += LnProbLogNormal_Mean_Var (1.0, newVar, rate[p->index]);
                    }
                }
            }
        }
...
int Move_RelaxedClockModel (Param *param, int chain, RandLong *seed, YFlt *lnPriorRatio, YFlt *lnProposalRatio, YFlt *mvp)
{
    ...
    int         i, *rclModel;
    YFlt      *mxvar, *mxRate, /* *brlens, */ igrvar, ilnvar, ratio;
    Tree        *t;
    TreeNode    *p = NULL;
    ModelInfo   *m;
    Model       *mp;
    ...
    if ((*rclModel) == RCL_IGR)
        ...
        for (i = 0; i < t->nNodes -2; i++)
            ...
            if (p->length > 0.0)  // not ancestral fossil
                ...
                (*lnPriorRatio) += LnProbLogNormal_Mean_Var (1.0, ilnvar, mxRate[p->index]);
                }
            }
...
#   endif
    ...
    if (isVPriorExp == YES)
        (*lnPriorRatio) = (mp->brlensExp) * (oldM - newM);
...
#   endif
    ...
    for (i=0; i<param->subParams[0]->nSubParams; i++)
        ...
        if (subParm->paramType == P_CPPEVENTS)
            ...
            /* no proposal ratio effect */
            /* prior ratio and update of effective evolutionary lengths */
            (*lnPriorRatio) -= LnProbLogNormal_Mean_LogVar (tk02Rate[u->index], nu*oldALength, tk02Rate[a->index]);
            (*lnPriorRatio) += LnProbLogNormal_Mean_LogVar (tk02Rate[u->index], nu*u->left->length, tk02Rate[u->left->index]);
            (*lnPriorRatio) -= LnProbLogNormal_Mean_LogVar (tk02Rate[u->index], nu*oldBLength, tk02Rate[b->index]);
            (*lnPriorRatio) += LnProbLogNormal_Mean_LogVar (tk02Rate[u->index], nu*u->right->length, tk02Rate[u->right->index]);
            (*lnPriorRatio) -= LnProbLogNormal_Mean_LogVar (tk02Rate[v->index], nu*oldCLength, tk02Rate[c->index]);
            (*lnPriorRatio) -= LnProbLogNormal_Mean_LogVar (tk02Rate[v->index], nu*oldULength, tk02Rate[u->index]);
            (*lnPriorRatio) += LnProbLogNormal_Mean_LogVar (tk02Rate[v->index], nu*v->left->length, tk02Rate[v->left->index]);
            (*lnPriorRatio) += LnProbLogNormal_Mean_LogVar (tk02Rate[v->index], nu*v->right->length, tk02Rate[v->right->index]);
            ...
            if (v->anc->anc != NULL && v->isDated == NO)
                {
                (*lnPriorRatio) -= LnProbLogNormal_Mean_LogVar (tk02Rate[w->index], nu*oldVLength, tk02Rate[v->index]);
                (*lnPriorRatio) += LnProbLogNormal_Mean_LogVar (tk02Rate[w->index], nu*v->length, tk02Rate[v->index]);
...
|
|--------------------------------------------------------------------*/
int Move_LSPR (Param *param, int chain, RandLong *seed, YFlt *lnPriorRatio, YFlt *lnProposalRatio, YFlt *mvp)
{
    /* Change branch lengths and topology (potentially) using SPR-type move 
    ...
    BitsLong    *pA, *pV, *pP;
    YFlt      x, minV, maxV, brlensExp=0.0, minLength=0.0, curLength=0.0, length=0.0,
...
#   endif
    ...
    if (!nSitesOfPat)
        {
        YvyraPrint ("%s   Problem allocating nSitesOfPat in Move_LSPR\n", spacer);
        free (tempCondLikes);
        free (tempCondLikePtr);
        return (ERROR);
        }
...
|
|--------------------------------------------------------------------*/
int Move_LSPR1 (Param *param, int chain, RandLong *seed, YFlt *lnPriorRatio, YFlt *lnProposalRatio, YFlt *mvp)
{
    /* Change branch lengths and topology (potentially) using SPR-type move 
       biased according to likelihood scores. NOT work for constrained trees. */
...
int Move_MixtureRates (Param *param, int chain, RandLong *seed, YFlt *lnPriorRatio, YFlt *lnProposalRatio, YFlt *mvp)
{
    ...
    YFlt      dirichletParameters[MAX_RATE_CATS], *newRates, *oldRates, *priorAlpha, sum, alphaPi, x, y, nStatesYFlt;
    
    /* get the values we need */
    nStates       = param->nSubValues;
    nStatesYFlt = (YFlt)(nStates);
    ...
    for (i=0; i<nStates; i++)
        dirichletParameters[i] = oldRates[i] * alphaPi / nStatesYFlt;
    ...
    do  {
        ...
        for (i=0; i<nStates; i++)
        {
            if (newRates[i] * nStatesYFlt < RATE_MIN)
            {
                isValid = NO;
                break;
            }
    ...
    qsort(newRates, (size_t)nStates, sizeof(YFlt), &cmpYFlt);
    for (i=0; i<nStates; i++)
        newRates[i] = newRates[i] * nStatesYFlt;
    
    /* get proposal ratio */
    /* we ignore the ordering factor, which is the same for new and old states */
    sum = 0.0;
    for (i=0; i<nStates; i++)
        sum += newRates[i]*alphaPi/nStatesYFlt;
    x = LnGamma(sum);
    for (i=0; i<nStates; i++)
        x -= LnGamma(newRates[i]*alphaPi/nStatesYFlt);
    for (i=0; i<nStates; i++)
        x += (newRates[i]*alphaPi/nStatesYFlt-1.0)*log(oldRates[i]/nStatesYFlt);
    sum = 0.0;
    for (i=0; i<nStates; i++)
        sum += oldRates[i]*alphaPi/nStatesYFlt;
    y = LnGamma(sum);
    for (i=0; i<nStates; i++)
        y -= LnGamma(oldRates[i]*alphaPi/nStatesYFlt);
    for (i=0; i<nStates; i++)
        y += (oldRates[i]*alphaPi/nStatesYFlt-1.0)*log(newRates[i]/nStatesYFlt);
    (*lnProposalRatio) = x - y;
    ...
    for (i=0; i<nStates; i++)
        x += (priorAlpha[i]-1.0)*log(newRates[i]/nStatesYFlt);
    for (i=0; i<nStates; i++)
        y += (priorAlpha[i]-1.0)*log(oldRates[i]/nStatesYFlt);
    (*lnPriorRatio) = x - y;
...
int Move_MixtureRates_Slider (Param *param, int chain, RandLong *seed, YFlt *lnPriorRatio, YFlt *lnProposalRatio, YFlt *mvp)
{
    ...
    YFlt      delta, *newRates, *oldRates, *priorAlpha, x, y, sum, min, max, nStatesYFlt;
    ...
    *lnProposalRatio = 0.0;
    ...
    x = (priorAlpha[i]-1.0)*log(newRates[i]/nStatesYFlt);
    x += (priorAlpha[j]-1.0)*log(newRates[j]/nStatesYFlt);
    y = (priorAlpha[i]-1.0)*log(oldRates[i]/nStatesYFlt);
    y += (priorAlpha[j]-1.0)*log(oldRates[j]/nStatesYFlt);
    (*lnPriorRatio) = x - y;
...
int Move_NNIClock (Param *param, int chain, RandLong *seed, YFlt *lnPriorRatio, YFlt *lnProposalRatio, YFlt *mvp)
{
    ...
    int         i, *nEvents, numFreeOld, numFreeNew;
    YFlt      x, *brlens, *rate=NULL, var=0.0, nu=0.0, oldALength, oldCLength;
...
#   endif
    ...
    if (numFreeNew != numFreeOld)
        (*lnProposalRatio) = log((YFlt)numFreeOld / (YFlt)numFreeNew);
    ...
    for (i=0; i<param->subParams[0]->nSubParams; i++)
        ...
        if (subParm->paramType == P_CPPEVENTS)
            ...
            rate = GetParamVals (subParm, chain, state[chain]);
            /* prior ratio and update of effective evolutionary lengths */
            (*lnPriorRatio) -= LnProbLogNormal_Mean_LogVar (rate[v->index], nu*oldALength, rate[a->index]);
            (*lnPriorRatio) += LnProbLogNormal_Mean_LogVar (rate[u->index], nu* a->length, rate[a->index]);
            (*lnPriorRatio) -= LnProbLogNormal_Mean_LogVar (rate[u->index], nu*oldCLength, rate[c->index]);
            (*lnPriorRatio) += LnProbLogNormal_Mean_LogVar (rate[v->index], nu* c->length, rate[c->index]);
            brlens = GetParamSubVals (subParm, chain, state[chain]);
            brlens[a->index] = a->length * (rate[a->index] + rate[a->anc->index])/2.0;
            brlens[c->index] = c->length * (rate[c->index] + rate[c->anc->index])/2.0;
            }
...
{
    YFlt      tuning, maxV, minV, oldM, newM, newMin, newMax, oldMin, oldMax;
    TreeNode    *p, *q;
    ModelParams *mp;
    Tree        *t;
    int isVPriorExp;
...
{
    int         i, *nEvents;
    YFlt      window, minDepth, maxDepth, oldDepth, newDepth, minL, minR,
    ...
    for (i=0; i<param->nSubParams; i++)
        ...
        if (subParm->paramType == P_CPPEVENTS)
            ...
            if (p->left != NULL)
                ...
                if (p->left->length > 0.0)
                    {
                    (*lnPriorRatio) -= LnProbLogNormal_Mean_LogVar (rate[p->index], nu*oldLeftLength, rate[p->left->index]);
                    (*lnPriorRatio) += LnProbLogNormal_Mean_LogVar (rate[p->index], nu*p->left->length, rate[p->left->index]);
                    brlens[p->left->index] = p->left->length * (rate[p->left->index]+rate[p->index])/2.0;
                    }
                if (p->right->length > 0.0)
                    {
                    (*lnPriorRatio) -= LnProbLogNormal_Mean_LogVar (rate[p->index], nu*oldRightLength, rate[p->right->index]);
                    (*lnPriorRatio) += LnProbLogNormal_Mean_LogVar (rate[p->index], nu*p->right->length, rate[p->right->index]);
                    brlens[p->right->index] = p->right->length * (rate[p->right->index]+rate[p->index])/2.0;
                    }
                }
            if (p->anc->anc != NULL)
                {
                (*lnPriorRatio) -= LnProbLogNormal_Mean_LogVar (rate[p->anc->index], nu*oldPLength, rate[p->index]);
                (*lnPriorRatio) += LnProbLogNormal_Mean_LogVar (rate[p->anc->index], nu*p->length, rate[p->index]);
...
int Move_Nu (Param *param, int chain, RandLong *seed, YFlt *lnPriorRatio, YFlt *lnProposalRatio, YFlt *mvp)
{
    ...
    int         i, j;
    YFlt      oldNu, newNu, minNu, maxNu, tuning, *rate;
    Model       *mp;
    TreeNode    *p;
    Tree        *t;
    ...
    for (i=0; i<param->nSubParams; i++)
        ...
        for (j=0; j<t->nNodes-2; j++)
            ...
            if (p->length > 0.0)  // not ancestral fossil
                {
                (*lnPriorRatio) -= LnProbLogNormal_Mean_LogVar (rate[p->anc->index], oldNu*p->length, rate[p->index]);
                (*lnPriorRatio) += LnProbLogNormal_Mean_LogVar (rate[p->anc->index], newNu*p->length, rate[p->index]);
                }
            }
        }
...
int Move_ParsEraser1 (Param *param, int chain, RandLong *seed, YFlt *lnPriorRatio, YFlt *lnProposalRatio, YFlt *mvp)
{
    ...
    YFlt      alphaPi, warp, minV, maxV, minP, maxP, newM, oldM, maxLen,
    ...
    *lnPriorRatio = *lnProposalRatio = 0.0;
    ...
    brlensCur = (YFlt *) SafeMalloc (8 * nSubTerminals * sizeof (YFlt));
    if (!brlensCur)
        {
        YvyraPrint ("%s   ERROR: Could not allocate brlensCur\n", spacer);
        goto errorExit;
        }
    ...
    if (!subtree->nodes || !subtree->intDownPass || !subtree->allDownPass)
        {
        YvyraPrint ("%s   ERROR: Could not allocate subtree\n", spacer);
        goto errorExit;
        }
    ...
    if (!subtree1->nodes || !subtree1->intDownPass || !subtree1->allDownPass)
        {
        YvyraPrint ("%s   ERROR: Could not allocate subtree1\n", spacer);
        goto errorExit;
        }
    ...
    if (!tInfo.leaf)
        {
        YvyraPrint ("%s   ERROR: Could not allocate tInfo.leaf\n", spacer);
        goto errorExit;
        }
    ...
    if (GetRandomEmbeddedSubtree (t, nSubTerminals, seed, &nEmbeddedTrees) == ERROR)
        {
        YvyraPrint ("%s   ERROR: Could not get subtree\n", spacer);
        goto errorExit;
        }
    ...
    if (GetRandomEmbeddedSubtree (t, nSubTerminals, seed, &i) == ERROR)
        {
        YvyraPrint ("%s   Could not count number of subtrees in Move_ParsEraser1\n", spacer);
        goto errorExit;
        }
    if (i != nEmbeddedTrees)
        (*lnProposalRatio) += log ((YFlt) nEmbeddedTrees / (YFlt) i);
...
int Move_ParsSPR (Param *param, int chain, RandLong *seed, YFlt *lnPriorRatio, YFlt *lnProposalRatio, YFlt *mvp)
{
    ...
    YFlt      x, minV, maxV, minLength=0.0, length=0.0,
...
#   endif
    ...
    if (!nSitesOfPat)
        {
        YvyraPrint ("%s   Problem allocating nSitesOfPat in Move_ParsSPR\n", spacer);
        return (ERROR);
        }
    for (i=0; i<numCompressedChars; i++)
        {
    ...
    (*lnProposalRatio) += log (x / c->length);
    if (isVPriorExp == YES)
        (*lnPriorRatio) += (mp->brlensExp) * (c->length - x);
    c->length = x;
    ...
    (*lnProposalRatio) += log (x / a->length);
    if (isVPriorExp == YES)
        (*lnPriorRatio) += (mp->brlensExp) * (a->length - x);
    a->length = x;
    ...
    (*lnProposalRatio) += log (x / u->length);
    if (isVPriorExp == YES)
        (*lnPriorRatio) += (mp->brlensExp) * (u->length - x);
    u->length = x;
...
int Move_ParsSPR1 (Param *param, int chain, RandLong *seed, YFlt *lnPriorRatio, YFlt *lnProposalRatio, YFlt *mvp)
{
    ...
    YFlt      x, minV, maxV, minLength=0.0, length=0.0, *parLength=NULL, prob, ran, tuning, warpFactor,
...
//  increaseProb = decreaseProb = mvp[1]; /* reweighting probabilities */
    ...
    parLength = (YFlt *) SafeCalloc ((size_t)nRoot * (size_t)nCrown, sizeof(YFlt));
...
outLoop:;
    ...
    if (moveInRoot == YES)  /* root part has changed */
        ...
        if (isVPriorExp == YES)
            (*lnPriorRatio) += (mp->brlensExp) * (newA->length - x);
        newA->length = x;
    ...
    if (moveInRoot == NO)  /* crown part has changed */
        ...
        if (isVPriorExp == YES)
            (*lnPriorRatio) += (mp->brlensExp) * (q->length - x);
        q->length = x;
        q->upDateTi = YES;
        ...
        if (isVPriorExp == YES)
            (*lnPriorRatio) += (mp->brlensExp) * (newB->length - x);
        newB->length = x;
        newB->upDateTi = YES;
        ...
        if (isVPriorExp == YES)
            (*lnPriorRatio) += (mp->brlensExp) * (newC->length - x);
        newC->length = x;
        newC->upDateTi = YES;
...
errorExit:
    YvyraPrint ("%s   Problem allocating memory in Move_ParsSPR1\n", spacer);
    free (parLength); free (pRoot); free (pCrown); free (nSitesOfPat);
...
errorExit:
    YvyraPrint ("%s   Problem allocating memory in Move_ParsSPR\n", spacer);
    free (parLength); free (pRoot); free (pCrown); free (nSitesOfPat);
...
int Move_ParsSPRClock (Param *param, int chain, RandLong *seed, YFlt *lnPriorRatio, YFlt *lnProposalRatio, YFlt *mvp)
{
    ...
    BitsLong    *pA, *pV, *pP, y[2];
    YFlt      x, oldBrlen=0.0, newBrlen=0.0, v1=0.0, v2=0.0, v3=0.0, v4=0.0, v5=0.0,
...
#   endif
    ...
    if (!nSitesOfPat)
        {
        YvyraPrint ("%s   Problem allocating nSitesOfPat in Move_ParsSPRClock\n", spacer);
        return (ERROR);
        }
    for (i=0; i<numCompressedChars; i++)
        {
    ...
    for (i=0; i<param->subParams[0]->nSubParams; i++)
        ...
        if (subParm->paramType == P_CPPEVENTS)
            ...
            if (n5 > 0)
                ...
                if (n4 > 0)
                    {
                    position[c->index] = (YFlt *) SafeRealloc ((void *) position[c->index], n4 * sizeof (YFlt));
                    rateMultiplier[c->index] = (YFlt *) SafeRealloc ((void *) rateMultiplier[c->index], n4 * sizeof (YFlt));
                    for (j=0; j<n4; j++)
            ...
            if (subParm->paramType == P_TK02BRANCHRATES)
                ...
                nu = *GetParamVals (modelSettings[subParm->relParts[0]].mixedvar, chain, state[chain]);
            rate = GetParamVals (subParm, chain, state[chain]);
            (*lnPriorRatio) -= LnProbLogNormal_Mean_LogVar(rate[u->anc->index], nu*(c->length+u->length), rate[c->index]);
            (*lnPriorRatio) += LnProbLogNormal_Mean_LogVar(rate[c->anc->index], nu*c->length, rate[c->index]);
            (*lnPriorRatio) += LnProbLogNormal_Mean_LogVar(rate[u->anc->index], nu*u->length, rate[u->index]);
            if (v->length > 0.0)
                (*lnPriorRatio) += LnProbLogNormal_Mean_LogVar(rate[v->anc->index], nu*v->length, rate[v->index]);
...
outLoop:;
    ...
    if (isVPriorExp == YES)
        (*lnPriorRatio) += (mp->brlensExp) * (v->length - x);
    v->length = x;
    v->upDateTi = YES;
...
errorExit:
    YvyraPrint ("%s   Problem allocating memory in Move_ParsTBR\n", spacer);
    free (parLength); free (pRoot); free (pCrown); free (nSitesOfPat);
...
int Move_Pinvar (Param *param, int chain, RandLong *seed, YFlt *lnPriorRatio, YFlt *lnProposalRatio, YFlt *mvp)
{
    ...
    YFlt          oldP, newP, window, minP, maxP, ran, lnInvarRatio, lnVarRatio;
    CLFlt           *nSitesOfPat;
    ModelParams     *mp;
    ModelInfo       *m;
...
int Move_PopSize_M (Param *param, int chain, RandLong *seed, YFlt *lnPriorRatio, YFlt *lnProposalRatio, YFlt *mvp)
{
    ...
    YFlt          *valPtr, oldN, newN, tuning, minN, maxN, ran, oldLnPrior=0.0, newLnPrior=0.0, growth,
                    oldT, newT, clockRate;
    ModelParams     *mp;
    ModelInfo       *m;
    Tree            *t;
    ...
    /* species tree coalescence prior removed: not supported in yvyra */
...
{
    int         i, isValid;
    YFlt      oldX, newX, minX, maxX, tuning, ran, factor;
...
int Move_RateMult_Dir (Param *param, int chain, RandLong *seed, YFlt *lnPriorRatio, YFlt *lnProposalRatio, YFlt *mvp)
{
    ...
    /* allocate memory */
    dirParm = (YFlt *) SafeCalloc (3*numCurrentDivisions, sizeof(YFlt));
    oldRate = dirParm + numCurrentDivisions;
    newRate = dirParm + 2*numCurrentDivisions;
...
int Move_RateMult_Slider (Param *param, int chain, RandLong *seed, YFlt *lnPriorRatio, YFlt *lnProposalRatio, YFlt *mvp)
{
    ...
    YFlt      delta, *value, *subValue, sum, numSites, *alphaDir, x, y,
...
int Move_RateShape_M (Param *param, int chain, RandLong *seed, YFlt *lnPriorRatio, YFlt *lnProposalRatio, YFlt *mvp)
{
    ...
    YFlt      oldA, newA, minA, maxA, ran, factor, tuning, *rates;
    ModelParams *mp;
    ...
    *lnProposalRatio = log(newA / oldA);
    ...
    if (isAPriorExp == NO)
        ...
        *lnPriorRatio = -(mp->shapeExp) * (newA - oldA);
...
int Move_Speciation (Param *param, int chain, RandLong *seed, YFlt *lnPriorRatio, YFlt *lnProposalRatio, YFlt *mvp)
{
    ...
    YFlt      *valPtr, oldL, newL, minL, maxL, *sR, *eR, sF, *fR, oldLnPrior, newLnPrior,
                window, clockRate;
    char        *sS;
    ModelParams *mp;
    ModelInfo   *m;
    ...
    *lnProposalRatio = 0.0;
    ...
    if (!strcmp(mp->clockPr,"Birthdeath"))
        ...
        *valPtr = newL;  // update with new value
        ...
        YvyraPrint ("%s   Move_Speciation not applicable\n", spacer);
        return (ERROR);
        }
    ...
    if (isLPriorExp == NO)
        ...
        *lnPriorRatio = newLnPrior - oldLnPrior - (mp->speciationExp)*(newL - oldL);
    ...
    *lnProposalRatio = log (newL / oldL);
    ...
    if (!strcmp(mp->clockPr,"Birthdeath"))
        ...
        *valPtr = newL;  // update with new value
        ...
        YvyraPrint ("%s   Move_Speciation_M not applicable\n", spacer);
        return (ERROR);
        }
...
int Move_Statefreqs (Param *param, int chain, RandLong *seed, YFlt *lnPriorRatio, YFlt *lnProposalRatio, YFlt *mvp)
{
    /* change pi */
    int         i, nStates, isValid;
    YFlt      dirichletParameters[64], *newPi, *oldPi, *priorAlpha, sum, alphaPi, x, y;
...
{
    int         i, j, nStates, isValid;
    YFlt      delta, *newPi, *oldPi, *priorAlpha, x, y, sum, min, max;
    ModelInfo *m;                            //added by SK to account for DIRPI parameters
...

{
	/* change pi */
	int			i, nStates, isValid;
	YFlt		dirichletParameters[64], *newPi, *oldPi, *priorAlpha, *temp, alphaPi, x, y;
    ModelInfo *m;                            
    ...
    temp = (YFlt *) SafeCalloc(nStates, sizeof (YFlt));
	if (!temp)
    {
		free (temp);
		return (ERROR);
...
int Move_Statefreqs_SplitMerge (Param *param, int chain, RandLong *seed, YFlt *lnPriorRatio, YFlt *lnProposalRatio, YFlt *mvp)
{
	...
	YFlt		dirichletParameters[64], *newPi, *oldPi, *priorAlpha, *newPiRoot, *oldPiRoot, *priorAlphaRoot, *temp, alphaPi, x, y;
...
int Move_StatefreqsSymDirMultistate (Param *param, int chain, RandLong *seed, YFlt *lnPriorRatio, YFlt *lnProposalRatio, YFlt *mvp)
{
    ...
    /* ideally, we would let the likelihood calculator deal with only the affected character
       but we do not have the mechanism for doing that in the current version of yvyra, so
       take the hit of updating all chars of the morph partition(s). */
    int     i, nStates, charIndex;
    YFlt  dirichletParameters[MAX_STD_STATES], symDirAlphai, *newPi, *oldPi, sum, alphaPi, x, y;
    Model   *mp;
...
int Move_TK02BranchRate (Param *param, int chain, RandLong *seed, YFlt *lnPriorRatio, YFlt *lnProposalRatio, YFlt *mvp)
{
    ...
    int         i;
    YFlt      newRate, oldRate, tuning, minR, maxR, nu, *tk02Rate, *brlens;
    TreeNode    *p = NULL;
    ModelInfo   *m;
    Tree        *t;
    TreeNode    *q;
    ...
    (*lnPriorRatio) -= LnProbLogNormal_Mean_LogVar (tk02Rate[p->anc->index], nu*p->length, oldRate);
    (*lnPriorRatio) += LnProbLogNormal_Mean_LogVar (tk02Rate[p->anc->index], nu*p->length, newRate);
    if (p->left != NULL)
        {
        if (p->left->length > 0.0)
            {
            (*lnPriorRatio) -= LnProbLogNormal_Mean_LogVar (oldRate, nu*p->left->length,  tk02Rate[p->left->index ]);
            (*lnPriorRatio) += LnProbLogNormal_Mean_LogVar (newRate, nu*p->left->length,  tk02Rate[p->left->index ]);
            }
        if (p->right->length > 0.0)
            {
            (*lnPriorRatio) -= LnProbLogNormal_Mean_LogVar (oldRate, nu*p->right->length, tk02Rate[p->right->index]);
            (*lnPriorRatio) += LnProbLogNormal_Mean_LogVar (newRate, nu*p->right->length, tk02Rate[p->right->index]);
            }
        }
...
int Move_TreeLen (Param *param, int chain, RandLong *seed, YFlt *lnPriorRatio, YFlt *lnProposalRatio, YFlt *mvp)
{
    ...
    YFlt      begin_tl, treescaler, tuning, maxV, minV;
    TreeNode    *p;
    ModelParams *mp;
    Tree        *t;
    ...
    /* update prior if exponential prior on branch lengths */
    if (param->paramId == BRLENS_EXP)
        (*lnPriorRatio) = (mp->brlensExp) * begin_tl * (1 - treescaler);
    /* Dirichlet or twoExp prior */
    else if (isVPriorExp > 1)
        (*lnPriorRatio) += LogDirPrior(t, mp, isVPriorExp);
...
int Move_TreeStretch (Param *param, int chain, RandLong *seed, YFlt *lnPriorRatio, YFlt *lnProposalRatio, YFlt *mvp)
{
    ...
    YFlt      minV, maxV, tuning, factor, lambda=0.0, x,
                *brlens=NULL, nu=0.0, var=0.0, *rate=NULL;
    TreeNode    *p, *q;
    ModelParams *mp;
    ModelInfo   *m;
...
int Move_WNBranchRate (Param *param, int chain, RandLong *seed, YFlt *lnPriorRatio, YFlt *lnProposalRatio, YFlt *mvp)
{
    ...
    int         i;
    YFlt      newRate, oldRate, tuning, minR, maxR, wnvar, *wnRate, *brlens;
    TreeNode    *p = NULL;
    ModelInfo   *m;
    Tree        *t;
    TreeNode    *q;
