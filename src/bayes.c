 ...
 *  Copyright (C) 2002-2023 John P. Huelsenbeck, Fredrik Ronquist et al.
 *
 *  John P. Huelsenbeck
...
int  InitializeYvyra (void);
void PrintHeader (void);
int  ReinitializeYvyra (void);

/* global variables, declared in this file */
BitsLong    bitsLongWithAllBitsSet;      /* BitsLong with all bits set, for bit ops       */
ModelParams defaultModel;                /* Default model; vals set in InitializeYvyra  */
...
YFlt      myStateInfo[7];              /* likelihood/prior/heat/ran/moveInfo vals of me              */
YFlt      partnerStateInfo[7];         /* likelihood/prior/heat/ran/moveInfo vals of partner         */
#endif
...
#   if defined (MPI_ENABLED)
    ...
    if (ierror != MPI_SUCCESS)
        {
        YvyraPrint ("%s   Problem initializing MPI\n", spacer);
        exit (1);
        }
    ierror = MPI_Comm_size(MPI_COMM_WORLD, &num_procs);
    if (ierror != MPI_SUCCESS)
        {
        YvyraPrint ("%s   Problem getting the number of processors\n", spacer);
        exit (1);
        }
    ierror = MPI_Comm_rank(MPI_COMM_WORLD, &proc_id);
    if (ierror != MPI_SUCCESS)
        {
        YvyraPrint ("%s   Problem getting processors rank\n", spacer);
        exit (1);
        }
#   endif
...
#ifdef UNIX_COMMAND_LINE_PARSING
{
    ...
    while ((ch = getopt(argc, argv, "hiIv")) != -1) {
        ...
        case 'h': /* help */
             ...
             * succesfully */
            puts("yvyra, Bayesian Phylogenetics for Linguistic Data\n");
            puts("Usage:");
            printf("\t%s [-i] [filename ...]\n", argv[0]);
            printf("\t%s -v\n", argv[0]);
            printf("\t%s -h\n", argv[0]);
    ...
    /* Display yvyra header *after* parsing the command line un Unix systems */
    PrintHeader();
    ...
    for (;;)
        ...
        if (nProcessedArgs < argc) 
...
#endif
                {
                /* Check for .yaml extension — convert to NEXUS first */
                const char *fname = argv[nProcessedArgs];
                int flen = (int)strlen(fname);
                if (flen > 5 && (strcmp(fname + flen - 5, ".yaml") == 0 ||
                                  strcmp(fname + flen - 4, ".yml") == 0))
                    {
                    char *nexus = YamlToNexus(fname);
                    if (nexus)
                        {
                        /* Write converted NEXUS to a temp file */
                        char tmpName[300];
                        snprintf(tmpName, sizeof(tmpName), "%.*s.nex",
                                 (int)(flen - (fname[flen-5] == '.' ? 5 : 4)), fname);
                        FILE *fp = fopen(tmpName, "w");
                        if (fp)
                            {
                            fprintf(fp, "%s", nexus);
                            fclose(fp);
                            YvyraPrint ("%s   Converted YAML to NEXUS: %s\n", spacer, tmpName);
                            sprintf(cmdStr, "Execute %s", tmpName);
                            }
                        else
                            sprintf(cmdStr, "Execute %s", fname);
                        free(nexus);
                        }
                    else
                        {
                        YvyraPrint ("%s   Error converting YAML file\n", spacer);
                        sprintf(cmdStr, "Execute %s", fname);
                        }
                    }
                else
            ...
            if (mode == NONINTERACTIVE)
                {
                YvyraPrint ("%s   Tasks completed, exiting program because mode is noninteractive\n", spacer);
                YvyraPrint ("%s   To return control to the command line after completion of file processing, \n", spacer);
                YvyraPrint ("%s   set mode to interactive with 'yvyra -i <filename>' (i is for interactive)\n", spacer);
                YvyraPrint ("%s   or use 'set mode=interactive'\n\n", spacer);
                return (NO_ERROR);
                }
...
#   if defined (MPI_ENABLED)
            if (proc_id == 0)
                ...
                if (fgets (cmdStr, CMD_STRING_LENGTH - 2, stdin) == NULL)
                    {
                    if (feof(stdin))
                        YvyraPrint ("%s   End of File encountered on stdin; quitting\n", spacer);
                    else
                        YvyraPrint ("%s   Could not read command from stdin; quitting\n", spacer);
                    strcpy (cmdStr,"quit;\n");
                    }
                }
            ...
            if (ierror != MPI_SUCCESS)
                {
                YvyraPrint ("%s   Problem broadcasting command string\n", spacer);
                }
#   else
#       ifdef HAVE_LIBREADLINE
            cmdStrP = readline("yvyra > ");
            if (cmdStrP!=NULL) 
                    {
...
#   endif
        ...
        if (cmdStr[0] != ';')
            ...
            if (CheckStringValidity (cmdStr) == ERROR)
                {
                YvyraPrint ("   Unknown character in command string\n\n");
                }
            else
                {
                ...
                if (message == ERROR && quitOnError == YES)
                    {
                    YvyraPrint ("%s   Will exit with signal 1 (error) because quitonerror is set to yes\n", spacer);
                    YvyraPrint ("%s   If you want control to be returned to the command line on error,\n", spacer);
                    YvyraPrint ("%s   use 'yvyra -i <filename>' (i is for interactive) or use 'set quitonerror=no'\n\n", spacer);
                    return (ERROR);
                    }
...
#   if defined (MPI_ENABLED)
                ...
                if (ierror != MPI_SUCCESS)
                    {
                    YvyraPrint ("%s   Problem at command barrier\n", spacer);
                    }
#   endif
...
#   if defined (MPI_ENABLED)
    ...
    if (ierror != MPI_SUCCESS)
        {
        YvyraPrint ("%s   Problem broadcasting seed\n", spacer);
        }
    ...
    if (ierror != MPI_SUCCESS)
        {
        YvyraPrint ("%s   Problem broadcasting swap seed\n", spacer);
        }
    ...
    if (ierror != MPI_SUCCESS)
        {
        YvyraPrint ("%s   Problem broadcasting run ID seed\n", spacer);
        }
...
#   endif
    ...
    inferSiteLikes = NO;                             /* set default inferSiteLikes                    */
    ...
    /* AA rate matrices removed: molecular models not supported in yvyra */
    ...
    strcpy(defaultModel.statefreqModel, "Stationary");  /* use stationary model                         */
    ...
    /* Prior for state frequencies at the root */
...
//  defaultModel.brlensDir[1] = 20.0;                   /* 2nd param of invGamDir prior  */
    ...
    strcpy(defaultModel.inferSiteLikes, "No");          /* do not output per-site lnLs                */
...
{
    YvyraPrint ("\n\n");
    YvyraPrint ("                              yvyra %s %s\n\n", VERSION_NUMBER, HOST_CPU);
    YvyraPrint ("              Bayesian Phylogenetics for Linguistic Data\n\n");
    YvyraPrint ("                   Based on MrBayes by Ronquist et al.\n\n");
#   if defined (MPI_ENABLED)
    YvyraPrint ("                             (Parallel version)\n");
    YvyraPrint ("                         (%d processors available)\n\n", num_procs);
#   endif
    YvyraPrint ("              Distributed under the GNU General Public License\n\n\n");
    YvyraPrint ("               Type \"help\" or \"help <command>\" for information\n");
    YvyraPrint ("                     on the commands that are available.\n\n");
    YvyraPrint ("                   Type \"about\" for authorship and general\n");
    YvyraPrint ("                       information about the program.\n\n\n");
}
