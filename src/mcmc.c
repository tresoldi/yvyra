...
YFlt    GetFitchPartials (ModelInfo *m, int chain, int source1, int source2, int destination);
void      GetStamp (void);
void      GetSwappers (int *swapA, int *swapB, int run);
void      GetTempDownPassSeq (TreeNode *p, int *i, TreeNode **dp);
int       GetTotalRateShifts (Model *mp, YFlt *shiftTimes);
YFlt    GibbsSampleGamma (int chain, int division, RandLong *seed);
...
YFlt    LogLike (int chain);
YFlt    LogPrior (int chain);
int       LnBirthDeathPriorPrRandom    (Tree *t, YFlt clockRate, YFlt *prob, YFlt sR, YFlt eR, YFlt sF);
int       LnBirthDeathPriorPrDiversity (Tree *t, YFlt clockRate, YFlt *prob, YFlt sR, YFlt eR, YFlt sF);
int       LnBirthDeathPriorPrCluster   (Tree *t, YFlt clockRate, YFlt *prob, YFlt sR, YFlt eR, YFlt sF);
int       LnFossilizedBDPriorFossilTip (Tree *t, YFlt clockRate, YFlt *prob, YFlt *sR, YFlt *eR, YFlt *fR, YFlt sF);
int       LnFossilizedBDPriorRandom    (Tree *t, YFlt clockRate, YFlt *prob, YFlt *lambda, YFlt *mu, YFlt *psi, YFlt *rho, YFlt *t_sl, int n_sl);
int       LnFossilizedBDPriorDiversity (Tree *t, YFlt clockRate, YFlt *prob, YFlt *lambda, YFlt *mu, YFlt *psi, YFlt *rho, YFlt *t_sl, int n_sl);
YFlt    LnP0 (YFlt t, YFlt l, YFlt m);
YFlt    LnP0Subsample (YFlt t, YFlt l, YFlt m, YFlt f);
YFlt    LnP1 (YFlt t, YFlt l, YFlt m);
YFlt    LnP1Subsample (YFlt t, YFlt l, YFlt m, YFlt f);
YFlt    LnP0_fossil (YFlt t, YFlt lambda, YFlt mu, YFlt psi, YFlt c1, YFlt c2);
YFlt    LnP1_fossil (YFlt t, YFlt rho, YFlt c1, YFlt c2);
YFlt    LnQi_fossil (YFlt t, YFlt *t_f, int sl, YFlt *c1, YFlt *c2);
YFlt    LnPi_fossil (YFlt t, YFlt *t_f, int sl, YFlt *c1, YFlt *c2, YFlt *lambda, YFlt *mu, YFlt *psi);
...
void      PrintTiProbs (CLFlt *tP, YFlt *bs, int nStates);
int       PrintTopConvInfo (void);
void      PrintToScreen (long long curGen, long long startGen, time_t endingT, time_t startingT);
int       PrintTree (long long curGen, Param *treeParam, int chain, int showBrlens, YFlt clockRate);
YFlt    PropAncFossil (Param *param, int chain);
#if defined (MPI_ENABLED)
...
int       SetMoves (void);
int       SetBinaryQMatrix (YFlt **a, int whichChain, int division);
int       SetNucQMatrix (YFlt **a, int n, int whichChain, int division, YFlt rateMult, YFlt *rA, YFlt *rS);
int       SetProteinQMatrix (YFlt **a, int n, int whichChain, int division, YFlt rateMult);
...
YFlt          *stdStateFreqs;              /* std char state frequencies                   */
...
YFlt          *curLnL = NULL;              /* stores log likelihood                        */
YFlt          *curLnPr = NULL;             /* stores log prior probability                 */
int             stepRelativeBurninSS;        /* Should we use relative burn in within each step or not    */
YFlt          powerSS;                     /* power (beta) in power posterior distribution used in SS   */
YFlt          *marginalLnLSS = NULL;       /* marginal likelihood obtained using steppingstone sampling */
YFlt          *stepAcumulatorSS = NULL;    /* accumulates likelihoods for current step in SS            */
YFlt          *stepScalerSS = NULL;        /* scaler of stepAcumulatorSS in log scale in SS             */
YFlt          *splitfreqSS = NULL;         /* array holding split frequencies for each step in SS       */
...
YFlt          *posSelProbs;                /* probs. for positive selection                */
...
YFlt          **rateProbs;                 /* pointers to rate probs used by adgamma model */
YFlt          *rateProbSpace;              /* space for rate probs used by adgamma model   */
int             rateProbRowSize;             /* size of rate probs for one chain one state   */
YFlt          **markovTi[MAX_SMALL_JUMP];  /* trans prob matrices used in calc of adgamma  */
YFlt          **markovTiN;                 /* trans prob matrices used in calc of adgamma  */
...
YFlt          *maxLnL0 = NULL;             /* maximum likelihood                           */
FILE            *fpMcmc = NULL;              /* pointer to .mcmc file                        */
FILE            **fpParm = NULL;             /* pointer to .p file(s)                        */
FILE            **fpSiteLnL = NULL;         /* pointer to .slk file(s)                      */
...
int AddToPrintString (char *tempStr)
{
    ...
    if (len1 + len2 + 5 > printStringSize)
        ...
        if (!printString)
            {
            YvyraPrint ("%s   Problem reallocating printString (%d)\n", spacer, printStringSize * sizeof(char));
            goto errorExit;
            }
        }
    ...
    for (i=0; i<numTopologies; i++)
        ...
        for (j=0; j<chainParams.numRuns; j++)
            ...
            if ((fp = OpenBinaryFileR (tempStr)) == NULL) 
                {
                YvyraPrint ("%s   Problem opening file %s.\n", spacer, tempStr);
                free (tempStr);
                return (ERROR);
                }
...
int AddTreeToPartitionCounters (Tree *tree, int treeId, int runId)
{
    ...
    for (i=0; i<tree->nIntNodes-1; i++)
        ...
        if ((partFreqTreeRoot[treeId] = AddPartition (partFreqTreeRoot[treeId], partition[p->index], runId)) == NULL)
            {
            YvyraPrint ("%s   Could not allocate space for new partition in AddTreeToPartitionCounters\n", spacer);
            return ERROR;
            }
        }
...
int AttemptSwap (int swapA, int swapB, RandLong *seed)
{
    ...
    YFlt          tempA, tempB, lnLikeA, lnLikeB, lnPriorA, lnPriorB, lnR, r,
            ...
            if (chainParams.isSS == YES && isSwapSuccessful == YES && (tempIdMy % chainParams.numChains == 0 || (areWeA == YES && chainId[whichElementA] % chainParams.numChains == 0) || (areWeA == NO && chainId[whichElementB] % chainParams.numChains == 0)))
                {
                    ...
                    if (tempIdMy % chainParams.numChains == 0)
                        ...
                        marginalLnLSS   [ run ] = (YFlt) 0.0;
                        stepAcumulatorSS[ run ] = (YFlt) 0.0;
                        stepScalerSS    [ run ] = (YFlt) 0.0;
                        }
                    else if ((int)partnerStateInfo[0] % chainParams.numChains == 0)
                        {
                        marginalLnLSS   [ run ] = (YFlt) partnerStateInfo[1];
                        stepAcumulatorSS[ run ] = (YFlt) partnerStateInfo[2];
                        stepScalerSS    [ run ] = (YFlt) partnerStateInfo[3];
                        }
                }
...
{
    YFlt delta, logTuning, newTuning;
        ...
        for (k=0; k<numParams; k++)
            ...
            if (p->paramType == P_TOPOLOGY)
                ...
                if (p->paramId == TOPOLOGY_RCL_FIXED ||
                    ...
                    {
                    YvyraPrint ("%s   Tree %s is fixed so a parsimony-based starting tree is not built\n", spacer, p->name);
                    return (NO_ERROR);
                    }
                /* constrained topology */
                else if (tree->nConstraints > 0)
                    {
                    YvyraPrint ("%s   Tree %s is constrained and parsimony-based starting trees are not implemented for constrained trees (yet)\n", spacer, p->name);
                    return (NO_ERROR);
                    }
                /* random topology */
                else
...
{
    int     i, j, n, min;
    YFlt  f, sum, sumsq, stdev;

    n = chainParams.numRuns;
    min = (int)(chainParams.minPartFreq * stat->numSamples);
    if ((YFlt)min != chainParams.minPartFreq * stat->numSamples)
        min++;
    ...
    for (i=0; i<n; i++)
        {
        f = (YFlt) (p->count[i]) / (YFlt) (stat->numSamples);
        sum += f;
        sumsq += f * f;
        }
    ...
    if (chainParams.allComps == YES)
        ...
        for (i=0; i<n; i++)
            ...
            for (j=i+1; j<n; j++)
                ...
                f = (YFlt) (p->count[j]) / (YFlt) (stat->numSamples);
                sum += f;
                sumsq += f * f;
...
void PartFreq (PFNODE *p, STATS *stat, int *ntrees)
{
    ...
    for (i=0; i<chainParams.numRuns; i++)
        {
        f = (YFlt)(p->count[i]) / (YFlt)ntrees[i];
        sum += f;
        sumsq += f * f;
        }
...
int CheckTemperature (void)
{
    if (chainParams.userDefinedTemps == YES)
        ...
        if (AreDoublesEqual(chainParams.userTemps[0], 1.0, ETA)==NO)
            {
            YvyraPrint ("%s   The first user-defined temperature must be 1.0.\n", spacer);
            return (ERROR);
            }
        }
...
#   endif
    ...
    for (n=0; n<chainParams.numRuns; n++)
        {
        SafeFclose (&fpParm[n]);
        if (fpSiteLnL && fpSiteLnL[n])
            SafeFclose (&fpSiteLnL[n]);
#if defined (PRINT_DUMP)
        SafeFclose (&fpDump[n]);
#endif
            ...
            for (j=0; j<2*numLocalTaxa-2; j++)
                ...
                if (p->nEvents[toState][j] != p->nEvents[fromState][j])
                    ...
                    if (p->nEvents[fromState][j] == 0)
                        ...
                        {
                        p->position[toState][j] = (YFlt *) SafeCalloc (p->nEvents[fromState][j], sizeof (YFlt));
                        p->rateMult[toState][j] = (YFlt *) SafeCalloc (p->nEvents[fromState][j], sizeof (YFlt));
                        }
                    else
                        {
                        p->position[toState][j] = (YFlt *) SafeRealloc ((void *)p->position[toState][j], p->nEvents[fromState][j] * sizeof (YFlt));
                        p->rateMult[toState][j] = (YFlt *) SafeRealloc ((void *)p->rateMult[toState][j], p->nEvents[fromState][j] * sizeof (YFlt));
                        }
...
#   endif
    ...
    if (setUpAnalysisSuccess == NO)
        {
        /* rebuild analysis (e.g. after ctype changes) */
        if (SetUpAnalysis(&seed) == ERROR)
            {
            YvyraPrint ("%s   The analysis could not be started because there was an error during its setup.\n", spacer);
            YvyraPrint ("%s   Refer to error messages printed during model set up to address the problem.\n", spacer);
            goto errorExit;
            }
        }
    ...
    YvyraPrint ("%s   Running Markov chain\n", spacer);

    /* Initialize estimated character weights if any */
    if (InitCharWeights() == ERROR)
        goto errorExit;
...
#   endif
    ...
    if (numUsedMoves == 0)
        {
        YvyraPrint ("%s   No move is currently switched on.\n", spacer);
        YvyraPrint ("%s   There must be at least one move to run an MCMC analysis.\n", spacer);
        YvyraPrint ("%s   Switch on moves using the 'propset' command.\n", spacer);
        goto errorExit;
        }
    ...
    /* BEST chain variables removed: species tree not supported in yvyra */

    /* Allocate per-site log-likelihood arrays if requested */
    for (i=0; i<numCurrentDivisions; i++)
        {
        ModelInfo *mi = &modelSettings[i];
        if (mi->printSiteLikes == YES)
            {
            mi->siteLnL = (YFlt *) SafeCalloc (mi->numChars - mi->numDummyChars, sizeof(YFlt));
            if (mi->siteLnL == NULL)
                {
                YvyraPrint ("%s   Problem allocating siteLnL\n", spacer);
                goto errorExit;
                }
            }
        else
    ...
    if (chainParams.isSS == YES)
        ...
        if (memAllocs[ALLOC_SS] == YES)
            {
            YvyraPrint ("%s   SS is already allocated\n", spacer);
            goto errorExit;
            }
        else if ((marginalLnLSS = (YFlt *) SafeCalloc (chainParams.numRuns, sizeof(YFlt))) == NULL)
            {
            YvyraPrint ("%s   Problem allocating marginalLnLSS\n", spacer);
            goto errorExit;
            }
        else if ((stepScalerSS = (YFlt *) SafeCalloc (chainParams.numRuns, sizeof(YFlt))) == NULL)
            {
            YvyraPrint ("%s   Problem allocating stepScalerSS\n", spacer);
            free (marginalLnLSS);
            goto errorExit;
            }
        else if ((stepAcumulatorSS = (YFlt *) SafeCalloc (chainParams.numRuns, sizeof(YFlt))) == NULL)
            {
            YvyraPrint ("%s   Problem allocating stepAcumulatorSS\n", spacer);
            free (stepScalerSS);
            free (marginalLnLSS);
            goto errorExit;
            }
        else if ((splitfreqSS = (YFlt *) SafeCalloc (chainParams.numStepsSS*numTopologies, sizeof(YFlt))) == NULL)
            {
            YvyraPrint ("%s   Problem allocating splitfreqSS\n", spacer);
            free (stepScalerSS);
            free (marginalLnLSS);
    ...
    if (chainParams.append == YES)
        ...
        YvyraPrint ("%s   Getting values from previous run\n", spacer);
        strcpy(inputFileName, chainParams.chainFileName);
        strcat(inputFileName, ".ckp");
        if (OpenTextFileR(inputFileName) == NULL)
            {
            YvyraPrint ("%s   Could not find the checkpoint file '%s'.\n", spacer, inputFileName);
            YvyraPrint ("%s   Make sure it is in the working directory.\n", spacer);
            goto errorExit;
            }
...
#   endif
        ...
        if (chainParams.isSS==YES && c!=EOF)
            ...
            if (fgets(strBuf,chainParams.numRuns*20,tempFile)==NULL)
                {
                YvyraPrint ("%s   Error: Reading SsAcumulators from .ckp file fails.\n", spacer);
                free(strBuf);
                goto errorExit;
                }
            ...
            for (run=0; run<chainParams.numRuns; run++)
                ...
                if (tmpcp == NULL)
                    {
                    YvyraPrint ("%s   Error: Not enough values in SsAcumulators comment of .ckp file.   \n", spacer);
                    free(strBuf);
                    goto errorExit;
                    }
...
#   endif
        if (numPreviousGen == 0)
            {
            YvyraPrint ("%s   Could not find the number of generations in previous run.\n", spacer);
            goto errorExit;
            }
        else if (numPreviousGen >= chainParams.numGen)
            {
            YvyraPrint ("%s   The specified number of generations (%lli) was already finished in\n", spacer, chainParams.numGen);
            YvyraPrint ("%s   the previous run you are trying to append to.\n", spacer);
            goto errorExit;
            }
        else
            YvyraPrint ("%s   Using samples up to generation %lli from previous analysis.\n", spacer, numPreviousGen);
        }
    else
        {
        /* New run */
        ...
        if (!strcmp(chainParams.startParams,"Reset"))
            {
            YvyraPrint ("%s   Resetting starting values for substitution model parameters\n", spacer);
        ...
        if (!strcmp(chainParams.startTree,"Random"))
            {
            YvyraPrint ("%s   Resetting starting trees and tree parameters\n", spacer);
            FillTreeParams (&seed, 0, numLocalChains);
            }
        else if (!strcmp(chainParams.startTree,"Parsimony"))
            {
            YvyraPrint ("%s   Rebuilding starting trees using random addition sequences and parsimony\n", spacer);
        ...
        if (chainParams.numStartPerts > 0)
            {
            YvyraPrint ("%s   Randomly perturbing starting trees\n", spacer);
            for (i=0; i<numTrees; i++)
                {
...
int DoMcmcp (void)
{
    ...
    YvyraPrint ("%s   Successfully set chain parameters\n", spacer);
        ...
        if (!strcmp(parmName, "Burninss"))
            ...
            if (expecting == Expecting(EQUALSIGN))
                ...
                if (negBurninss == NO)
                    ...
                    chainParams.burninSS = -tempI;
                YvyraPrint ("%s   Setting burnin for stepping-stone sampling to %d\n", spacer, chainParams.burninSS);
            ...
            if (expecting == Expecting(EQUALSIGN))
                ...
                YvyraPrint ("%s   Setting number of steps in stepping-stone sampling to %d\n", spacer, chainParams.numStepsSS);
                expecting = Expecting(PARAMETER) | Expecting(SEMICOLON);
                }
            else 
                {
            ...
            if (expecting == Expecting(EQUALSIGN))
                ...
                if (IsArgValid(tkn, tempStr) == NO_ERROR)
                    ...
                    {
                    YvyraPrint ("%s   Invalid argument for FromPrior parameter\n", spacer);
                    return (ERROR);
                    }
                YvyraPrint ("%s   Setting FromPrior=%s\n", spacer, (chainParams.startFromPriorSS==YES)?"Yes":"No");
                expecting = Expecting(PARAMETER) | Expecting(SEMICOLON);
                }
            else 
                {
            ...
            if (expecting == Expecting(EQUALSIGN))
                ...
                YvyraPrint ("%s   Setting alpha in stepping-stone sampling to %lf\n", spacer, chainParams.alphaSS);
                expecting = Expecting(PARAMETER) | Expecting(SEMICOLON);
                }
            else 
                {
    ...
    *tempStr='\0';
    ...
    if (expecting == Expecting(PARAMETER))
        ...
        if (!strcmp(parmName, "Seed"))
            {
                YvyraPrint ("%s   Error: Setting \"Seed\" in Mcmc command is deprecated. Use \"set\" command instead.\n", spacer);
                YvyraPrint ("%s   For more information type \"help set\";\n", spacer);
                free (tempStr);
                return (ERROR);
            /*
            if (expecting == Expecting(EQUALSIGN))
        ...
        /* set Swapseed (global variable swapSeed) ***************************************************************/
            ...
            {
                YvyraPrint ("%s   Error: Setting \"Swapseed\" in Mcmc command is deprecated. Use \"set\" command instead.\n", spacer);
                YvyraPrint ("%s   For more information type \"help set\";\n", spacer);
                free (tempStr);
                return (ERROR);
                /*
        ...
        /* this setting is provided for GRID use only, so that identical runs can be generated */
            ...
            if (expecting == Expecting(EQUALSIGN))
                ...
                sscanf (tkn, "%d", &tempI);
                runIDSeed = tempI;
                YvyraPrint ("%s   Setting run ID [stamp] seed to %ld [for GRID use]\n", spacer, runIDSeed);
                expecting = Expecting(PARAMETER) | Expecting(SEMICOLON);
                }
            else
                {
        ...
        /* set Ngen (numGen) ******************************************************************/
            ...
            if (expecting == Expecting(EQUALSIGN))
                ...
                if (tempL < 1)
                    {
                    YvyraPrint ("%s   Too few generations\n", spacer);
                    return (ERROR);
                    }
                if ( tempL / chainParams.sampleFreq > INT_MAX )
                    {
                    YvyraPrint ("%s   Maximum %d samples allowed. Decrease 'ngen' or increase 'samplefreq'.\n", spacer, INT_MAX);
                    return (ERROR);
                    }
                chainParams.numGen = tempL;
                YvyraPrint ("%s   Setting number of generations to %lli\n", spacer, chainParams.numGen);
                expecting = Expecting(PARAMETER) | Expecting(SEMICOLON);
                }
            else
                {
        ...
        /* set Samplefreq (sampleFreq) ********************************************************/
            ...
            if (expecting == Expecting(EQUALSIGN))
                ...
                if (tempI < 1)
                    {
                    YvyraPrint ("%s   Sampling chain too infrequently\n", spacer);
                    free (tempStr);
                    return (ERROR);
                    }
                ...
                YvyraPrint ("%s   Setting sample frequency to %d\n", spacer, chainParams.sampleFreq);
                expecting = Expecting(PARAMETER) | Expecting(SEMICOLON);
                }
            else
                {
        ...
        /* set Printfreq (printFreq) **********************************************************/
            ...
            if (expecting == Expecting(EQUALSIGN))
                ...
                if (tempI < 1)
                    {
                    YvyraPrint ("%s   Printing to screen too infrequently\n", spacer);
                    free(tempStr);
                    return (ERROR);
                    }
                chainParams.printFreq = tempI;
                YvyraPrint ("%s   Setting print frequency to %d\n", spacer, chainParams.printFreq);
                expecting = Expecting(PARAMETER) | Expecting(SEMICOLON);
                }
            else
                {
        ...
        /* set Printmax (printMax) **********************************************************/
            ...
            if (expecting == Expecting(EQUALSIGN))
                ...
                if (tempI < 1)
                    {
                    YvyraPrint ("%s   You need to print at least one chain\n", spacer);
                    return (ERROR);
                    }
                chainParams.printMax = tempI;
                YvyraPrint ("%s   Setting maximum number of chains to print to screen to %d\n", spacer, chainParams.printMax);
                expecting = Expecting(PARAMETER) | Expecting(SEMICOLON);
                }
            else
                {
        ...
        /* set Printall (printAll) ********************************************************/
            ...
            if (expecting == Expecting(EQUALSIGN))
                ...
                if (IsArgValid(tkn, tempStr) == NO_ERROR)
                    ...
                    {
                    YvyraPrint ("%s   Invalid argument for Printall\n", spacer);
                    free (tempStr);
                    return (ERROR);
                    }
                if (chainParams.allChains == YES)
                    YvyraPrint ("%s   Printing all chains to screen\n", spacer);
                else
                    YvyraPrint ("%s   Printing only cold chains to screen\n", spacer);
                expecting = Expecting(PARAMETER) | Expecting(SEMICOLON);
                }
            else
                {
        ...
        /* set Swapfreq (swapFreq) ************************************************************/
            ...
            if (expecting == Expecting(EQUALSIGN))
                ...
                if (tempI < 1)
                    {
                    YvyraPrint ("%s   Swapping states too infrequently\n", spacer);
                    free(tempStr);
                    return (ERROR);
                    }
                chainParams.swapFreq = tempI;
                YvyraPrint ("%s   Setting swap frequency to %d\n", spacer, chainParams.swapFreq);
                expecting = Expecting(PARAMETER) | Expecting(SEMICOLON);
                }
            else
                {
        ...
        /* set Nswaps (numSwaps) ************************************************************/
            ...
            if (expecting == Expecting(EQUALSIGN))
                ...
                if (tempI < 1)
                    {
                    YvyraPrint ("%s   There must be at least one swap per swapping cycle\n", spacer);
                    free (tempStr);
                    return (ERROR);
                    }
                chainParams.numSwaps = tempI;
                YvyraPrint ("%s   Setting number of swaps per swapping cycle to %d\n", spacer, chainParams.numSwaps);
                expecting = Expecting(PARAMETER) | Expecting(SEMICOLON);
                }
            else
                {
        ...
        /* set Allchains (allChains) ********************************************************/
            ...
            if (expecting == Expecting(EQUALSIGN))
                ...
                if (IsArgValid(tkn, tempStr) == NO_ERROR)
                    ...
                    {
                    YvyraPrint ("%s   Invalid argument for Allchains\n", spacer);
                    free (tempStr);
                    return (ERROR);
                    }
                if (chainParams.allChains == YES)
                    YvyraPrint ("%s   Calculating MCMC diagnostics for all chains\n", spacer);
                else
                    YvyraPrint ("%s   Calculating MCMC diagnostics only for cold chain(s)\n", spacer);
                expecting = Expecting(PARAMETER) | Expecting(SEMICOLON);
                }
            else
                {
        ...
        /* set Allcomps (allComps) ************************************************************/
            ...
            if (expecting == Expecting(EQUALSIGN))
                ...
                if (IsArgValid(tkn, tempStr) == NO_ERROR)
                    ...
                    {
                    YvyraPrint ("%s   Invalid argument for Allcomps\n", spacer);
                    free (tempStr);
                    return (ERROR);
                    }
                if (chainParams.allComps == YES)
                    YvyraPrint ("%s   Calculating MCMC diagnostics for all pairwise run comparisons\n", spacer);
                else
                    YvyraPrint ("%s   Only calculating overall MCMC diagnostics\n", spacer);
                expecting = Expecting(PARAMETER) | Expecting(SEMICOLON);
                }
            else
                {
        ...
        /* set Mcmcdiagn (mcmcDiagn) ********************************************************/
            ...
            if (expecting == Expecting(EQUALSIGN))
                ...
                if (IsArgValid(tkn, tempStr) == NO_ERROR)
                    ...
                    {
                    YvyraPrint ("%s   Invalid argument for MCMC diagnostics\n", spacer);
                    free(tempStr);
                    return (ERROR);
                    }
                if (chainParams.mcmcDiagn == YES)
                    YvyraPrint ("%s   Setting calculation of MCMC diagnostics ('Mcmcdiagn') to yes\n", spacer);
                else
                    YvyraPrint ("%s   Setting calculation of MCMC diagnostics ('Mcmcdiagn') to no\n", spacer);
                expecting = Expecting(PARAMETER) | Expecting(SEMICOLON);
                }
            else
                {
        ...
        /* set Diagnfreq (diagnFreq) ************************************************************/
            ...
            if (expecting == Expecting(EQUALSIGN))
                ...
                if (tempI < 1)
                    {
                    YvyraPrint ("%s   Diagnosing MCMC behavior too infrequently\n", spacer);
                    free(tempStr);
                    return (ERROR);
                    }
                chainParams.diagnFreq = tempI;
                YvyraPrint ("%s   Setting diagnosing frequency to %d\n", spacer, chainParams.diagnFreq);
                expecting = Expecting(PARAMETER) | Expecting(SEMICOLON);
                }
            else 
                {
        ...
        /* set Savetrees (saveTrees) ********************************************************/
            ...
            if (expecting == Expecting(EQUALSIGN))
                ...
                if (IsArgValid(tkn, tempStr) == NO_ERROR)
                    ...
                    {
                    YvyraPrint ("%s   Invalid argument for Savetrees\n", spacer);
                    free(tempStr);
                    return (ERROR);
                    }
                if (chainParams.saveTrees == YES)
                    YvyraPrint ("%s   Saving trees for MCMC diagnostics in memory (if needed)\n", spacer);
                else
                    YvyraPrint ("%s   Not saving trees for MCMC diagnostics in memory\n", spacer);
                expecting = Expecting(PARAMETER) | Expecting(SEMICOLON);
                }
            else
                {
        ...
        /* set Diagnstat (diagnStat) ********************************************************/
            ...
            if (expecting == Expecting(EQUALSIGN))
                ...
                if (chainParams.diagnStat == AVGSTDDEV)
                    YvyraPrint ("%s   Setting diagnostics statistic to Avgstddev\n", spacer);
                else /* if (chainParams.diagnStat == MAXSTDDEV) */
                    YvyraPrint ("%s   Setting diagnostics statistic to Maxstddev\n", spacer);
                expecting = Expecting(PARAMETER) | Expecting(SEMICOLON);
                }
            else
                {
        ...
        /* set Checkpoint (checkPoint) ********************************************************/
            ...
            if (expecting == Expecting(EQUALSIGN))
                ...
                if (IsArgValid(tkn, tempStr) == NO_ERROR)
                    ...
                    {
                    YvyraPrint ("%s   Invalid argument for 'Checkpoint' (check-pointing)\n", spacer);
                    free(tempStr);
                    return (ERROR);
                    }
                if (chainParams.checkPoint == YES)
                    YvyraPrint ("%s   Setting check-pointing ('Checkpoint') to yes\n", spacer);
                else
                    YvyraPrint ("%s   Setting check-pointing ('Checkpoint') to no\n", spacer);
                expecting = Expecting(PARAMETER) | Expecting(SEMICOLON);
                }
            else
                {
        ...
        /* set Checkfreq (checkFreq) ************************************************************/
            ...
            if (expecting == Expecting(EQUALSIGN))
                ...
                if (tempI < 100)
                    {
                    YvyraPrint ("%s   Check-pointing frequency must be at least 100\n", spacer);
                    free(tempStr);
                    return (ERROR);
                    }
                chainParams.checkFreq = tempI;
                YvyraPrint ("%s   Setting check-pointing frequency to %d\n", spacer, chainParams.checkFreq);
                expecting = Expecting(PARAMETER) | Expecting(SEMICOLON);
                }
            else 
                {
        ...
        /* set Minpartfreq (minPartFreq) ************************************************************/
            ...
            if (expecting == Expecting(EQUALSIGN))
                ...
                if (tempD < 0.01)
                    {
                    YvyraPrint ("%s   Minimum partition frequency too low (< 0.01)\n", spacer);
                    free (tempStr);
                    return (ERROR);
                    }
                if (tempD > 0.8)
                    {
                    YvyraPrint ("%s   Minimum partition frequency too high (> 0.8)\n", spacer);
                    free (tempStr);
                    return (ERROR);
                    }
                chainParams.minPartFreq = tempD;
                YvyraPrint ("%s   Setting minimum partition frequency to %.2f\n", spacer, chainParams.minPartFreq);
                expecting = Expecting(PARAMETER) | Expecting(SEMICOLON);
                }
            else 
                {
        ...
        /* set Nruns (numRuns) ****************************************************************/
            ...
            if (expecting == Expecting(EQUALSIGN))
                ...
                if (tempI < 1)
                    {
                    YvyraPrint ("%s   Too few runs (minimum of 1 run)\n", spacer);
                    free(tempStr);
                    return (ERROR);
                    }
                if (tempI > MAX_RUNS)
                    {
                    YvyraPrint ("%s   Too many runs (maximum of %d runs)\n", spacer, MAX_RUNS);
                    free(tempStr);
                    return (ERROR);
                    }
                ...
                fileNameChanged = YES;
                YvyraPrint ("%s   Setting number of runs to %d\n", spacer, chainParams.numRuns);
                expecting = Expecting(PARAMETER) | Expecting(SEMICOLON);
                }
            else
                {
        ...
        /* set Nchains (numChains) ************************************************************/
            ...
            if (expecting == Expecting(EQUALSIGN))
                ...
                if (tempI < 1)
                    {
                    YvyraPrint ("%s   Too few chains (minimum of 1 chain)\n", spacer);
                    free(tempStr);
                    return (ERROR);
                    }
                if (tempI > MAX_CHAINS)
                    {
                    YvyraPrint ("%s   Too many chains (maximum of %d chains)\n", spacer, MAX_CHAINS);
                    free(tempStr);
                    return (ERROR);
                    }
                ...
                YvyraPrint ("%s   Setting number of chains to %d\n", spacer, chainParams.numChains);
                expecting = Expecting(PARAMETER) | Expecting(SEMICOLON);
                }
            else
                {
        ...
        /* set Temp (chainTemp) ***************************************************************/
            ...
            if (expecting == Expecting(EQUALSIGN))
                ...
                {
                YvyraPrint ("%s   Setting user-defined temperatures\n", spacer);
                expecting = Expecting(PARAMETER) | Expecting(SEMICOLON);
                }
                ...
                if (chainParams.userDefinedTemps == NO)
                    ...
                    YvyraPrint ("%s   Setting heating parameter to %lf\n", spacer, chainParams.chainTemp);
                    expecting = Expecting(PARAMETER) | Expecting(SEMICOLON);
                    }
                else
                    {
                    if (tempIndex >= MAX_CHAINS)
                        {
                        YvyraPrint ("%s   Too many user-defined temperatures (%d maximum)\n", spacer, MAX_CHAINS);
                        free(tempStr);
                        return (ERROR);
                        }
                    sscanf (tkn, "%lf", &tempD);
            ...
            if (expecting == Expecting(EQUALSIGN))
                ...
                if (whichReweightNum < 2)
                    ...
                    if (tempD < 0.0 || tempD > 100.0)
                        {
                        YvyraPrint ("%s   The reweighting parameter must be between 0 and 100\n", spacer);
                    ...
                    if (tempD <= 0.0 || tempD > 1.0)
                        {
                        YvyraPrint ("%s   The reweighting increment must be between 0 and 1\n", spacer);
                        chainParams.weightScheme[0] = chainParams.weightScheme[1] = 0.0;
                        chainParams.weightScheme[2] = 1.0;
                        free(tempStr);
                        return (ERROR);
                ...
                if (whichReweightNum == 0)
                    ...
                    if (chainParams.weightScheme[0] + chainParams.weightScheme[1] > 100.0)
                        {
                        YvyraPrint ("%s   The sum of the reweighting parameters cannot exceed 100 %%\n", spacer);
                        chainParams.weightScheme[0] = chainParams.weightScheme[1] = 0.0;
                        chainParams.weightScheme[2] = 1.0;
                        free(tempStr);
                        return (ERROR);
                ...
                if (chainParams.weightScheme[0] >= 100.0)
                    {
                    YvyraPrint ("%s   Cannot decrease weight of all characters\n", spacer);
                    chainParams.weightScheme[0] = chainParams.weightScheme[1] = 0.0;
                    chainParams.weightScheme[2] = 1.0;
                    free(tempStr);
                    return (ERROR);
                    }
                YvyraPrint ("%s   Setting reweighting parameter to (%1.2lf v, %1.2lf ^) increment = %1.2lf\n", 
        ...
        /* set Filename (chainFileName) *******************************************************/
            ...
            if (expecting == Expecting(EQUALSIGN))
                ...
                if (strlen(tempStr)>99)
                    {
                    YvyraPrint ("%s   Maximum allowed length of chain file name is 99 characters. The given name:\n", spacer);
                    YvyraPrint ("%s      '%s'\n", spacer,tempStr);
        ...
        /* set Burnin (chainBurnIn) ***********************************************************/
            ...
            if (expecting == Expecting(EQUALSIGN))
                ...
                YvyraPrint ("%s   Setting chain burn-in to %d\n", spacer, chainParams.chainBurnIn);
                expecting = Expecting(PARAMETER) | Expecting(SEMICOLON);
                }
            else
                {
        ...
        /* set Stoprule (stopRule) ********************************************************/
            ...
            if (expecting == Expecting(EQUALSIGN))
                ...
                if (IsArgValid(tkn, tempStr) == NO_ERROR)
                    ...
                    {
                    YvyraPrint ("%s   Invalid argument for Stoprule\n", spacer);
                    free(tempStr);
                    return (ERROR);
                    }
                if (chainParams.stopRule == YES)
                    YvyraPrint ("%s   Using stopping rule.\n", spacer);
                else
                    YvyraPrint ("%s   Not using stopping rule.\n", spacer);
                expecting = Expecting(PARAMETER) | Expecting(SEMICOLON);
                }
            else
                {
        ...
        /* set Stopval (stopVal) ************************************************************/
            ...
            if (expecting == Expecting(EQUALSIGN))
                ...
                if (tempD < 0.000001)
                    {
                    YvyraPrint ("%s   Stop value too low (< 0.000001)\n", spacer);
                    free(tempStr);
                    return (ERROR);
                    }
                if (tempD > 0.20)
                    {
                    YvyraPrint ("%s   Stop value too high (> 0.20)\n", spacer);
                    free(tempStr);
                    return (ERROR);
                    }
            ...
            if (expecting == Expecting(EQUALSIGN))
                ...
                if (IsArgValid(tkn, tempStr) == NO_ERROR)
                    ...
                    if (!strcmp(tempStr,"User"))
                        {
                        YvyraPrint ("%s   The 'user' setting of 'Starttree' is deprecated. Set starting trees using 'Startvals' instead.\n", spacer);
                        free (tempStr);
                        return (ERROR);
                        }
                    else
                    ...
                    {
                    YvyraPrint ("%s   Invalid 'Starttree' argument '%s'. This is a bug -- please report.\n", spacer, tempStr);
                    free(tempStr);
                    return (ERROR);
                    }
                YvyraPrint ("%s   Setting 'Starttree' to \"%s\"\n", spacer, chainParams.startTree);
                expecting = Expecting(PARAMETER) | Expecting(SEMICOLON);
                }
            else
                {
        ...
        /* set Startingtrees (deprecated) **************************************************/
            ...
            {
            free (tempStr);
            YvyraPrint ("%s   Parameter 'Startingtrees' is deprecated. Use the 'Starttree' parameter or the 'Startvals' command instead.\n", spacer);
            return (ERROR);
            }
        /* set Nperts (numStartPerts) *********************************************************/
            ...
            if (expecting == Expecting(EQUALSIGN))
                ...
                YvyraPrint ("%s   Setting number of perturbations to start tree to %d\n", spacer, chainParams.numStartPerts);
                expecting = Expecting(PARAMETER) | Expecting(SEMICOLON);
                }
            else
                {
        ...
        /* set Startparams (startParams) **************************************************/
            ...
            if (expecting == Expecting(EQUALSIGN))
                ...
                if (IsArgValid(tkn, tempStr) == NO_ERROR)
                    ...
                    {
                    YvyraPrint ("%s   Invalid 'Startparams' argument\n", spacer);
                    free(tempStr);
                    return (ERROR);
                    }
                YvyraPrint ("%s   Setting 'Startparams' to \"%s\"\n", spacer, chainParams.startParams);
                expecting = Expecting(PARAMETER) | Expecting(SEMICOLON);
                }
            else
                {
            ...
            if (expecting == Expecting(EQUALSIGN))
                ...
                if (IsArgValid(tkn, tempStr) == NO_ERROR)
                    ...
                    if (!strcmp(tempStr, "Yes"))
                        ...
                        YvyraPrint ("%s   WARNING: Ignoring savebrlens setting; since version 3.2, branch lengths are always saved\n", spacer);
                    }
                else
                    {
                    YvyraPrint ("%s   Invalid argument for savebrlens\n", spacer);
                    free(tempStr);
                    return (ERROR);
                    }
                expecting = Expecting(PARAMETER) | Expecting(SEMICOLON);
        ...
        /* set Redirect (redirect) ********************************************************/
            ...
            if (expecting == Expecting(EQUALSIGN))
                ...
                if (IsArgValid(tkn, tempStr) == NO_ERROR)
                    ...
                    {
                    YvyraPrint ("%s   Invalid argument for redirecting output\n", spacer);
                    free(tempStr);
                    return (ERROR);
                    }
                if (chainParams.redirect == YES)
                    YvyraPrint ("%s   Setting program to redirect output\n", spacer);
                else
                    YvyraPrint ("%s   Setting program not to redirect output\n", spacer);
                expecting = Expecting(PARAMETER) | Expecting(SEMICOLON);
                }
            else
                {
        ...
        /* set Data (runWithData) ************************************************************/
            ...
            if (expecting == Expecting(EQUALSIGN))
                ...
                if (IsArgValid(tkn, tempStr) == NO_ERROR)
                    ...
                    {
                    YvyraPrint ("%s   Invalid argument for Data\n", spacer);
                    free(tempStr);
                    return (ERROR);
                    }
                if (chainParams.runWithData == NO)
                    YvyraPrint ("%s   Running without data (WARNING: use this only for checking priors!)\n", spacer);
                else
                    YvyraPrint ("%s   Running with data (standard analysis)\n", spacer);
                expecting = Expecting(PARAMETER) | Expecting(SEMICOLON);
                }
            else
                {
        ...
        /* set Ordertaxa (chainParams.orderTaxa) *********************************************/
            ...
            if (expecting == Expecting(EQUALSIGN))
                ...
                if (IsArgValid(tkn, tempStr) == NO_ERROR)
                    ...
                    {
                    YvyraPrint ("%s   Invalid argument for ordertaxa\n", spacer);
                    free(tempStr);
                    return (ERROR);
                    }
                if (chainParams.orderTaxa == YES)
                    YvyraPrint ("%s   Setting ordertaxa to yes\n", spacer);
                else
                    YvyraPrint ("%s   Setting ordertaxa to no\n", spacer);
                expecting = Expecting(PARAMETER) | Expecting(SEMICOLON);
                }
            else
                {
        ...
        /* set Append (chainParams.append) *********************************************/
            ...
            if (expecting == Expecting(EQUALSIGN))
                ...
                if (IsArgValid(tkn, tempStr) == NO_ERROR)
                    ...
                    {
                    YvyraPrint ("%s   Invalid argument for append\n", spacer);
                    free(tempStr);
                    return (ERROR);
                    }
                if (chainParams.append == YES)
                    YvyraPrint ("%s   Setting append to yes\n", spacer);
                else
                    YvyraPrint ("%s   Setting append to no\n", spacer);
                expecting = Expecting(PARAMETER) | Expecting(SEMICOLON);
                }
            else
                {
        ...
        /* set Autotune (chainParams.autotune) *********************************************/
            ...
            if (expecting == Expecting(EQUALSIGN))
                ...
                if (IsArgValid(tkn, tempStr) == NO_ERROR)
                    ...
                    {
                    YvyraPrint ("%s   Invalid argument for Autotune\n", spacer);
                    free(tempStr);
                    return (ERROR);
                    }
                if (chainParams.autotune == YES)
                    YvyraPrint ("%s   Setting Autotune to yes\n", spacer);
                else
                    YvyraPrint ("%s   Setting Autotune to no\n", spacer);
                expecting = Expecting(PARAMETER) | Expecting(SEMICOLON);
                }
            else
                {
        ...
        /* set Tunefreq (tuneFreq) ************************************************************/
            ...
            if (expecting == Expecting(EQUALSIGN))
                ...
                if (tempI < 10)
                    {
                    YvyraPrint ("%s   Autotuning frequency must be at least 10\n", spacer);
                    free(tempStr);
                    return (ERROR);
                    }
                chainParams.tuneFreq = tempI;
                YvyraPrint ("%s   Setting autotuning frequency to %d\n", spacer, chainParams.tuneFreq);
                expecting = Expecting(PARAMETER) | Expecting(SEMICOLON);
                }
            else 
                {
        ...
        /* set Swapadjacent (swapAdjacentOnly) **************************************************/
            ...
            if (expecting == Expecting(EQUALSIGN))
                ...
                if (IsArgValid(tkn, tempStr) == NO_ERROR)
                    ...
                    {
                    YvyraPrint ("%s   Invalid argument for Swapadjacent\n", spacer);
                    free(tempStr);
                    return (ERROR);
                    }
                if (chainParams.swapAdjacentOnly == YES)
                    YvyraPrint ("%s   Setting program to attempt swaps only between chains of adjacent temperatures\n", spacer);
                else
                    YvyraPrint ("%s   Setting program to attempt all possible swaps between chains\n", spacer);
                expecting = Expecting(PARAMETER) | Expecting(SEMICOLON);
                }
            else
                {
...
int DoSs (void)
{
    ...
    if (chainParams.numGen/chainParams.sampleFreq <= chainParams.burninSS)
        ...
        YvyraPrint ("%s      ERROR: Burnin %d samples is too large compared with requested total %d samples (%lli generations).\n", spacer ,chainParams.burninSS, chainParams.numGen/chainParams.sampleFreq, chainParams.numGen);
        return ERROR;
        }
        ...
        if (proc_id == 0)
            ...
            do
                ...
                if (additionalCycles < 0)
                    YvyraPrint ("%s      Number must be greater than or equal to 0: ", spacer);
                else
                    YvyraPrint ("%s      Additional number of generations: ", spacer);
...
int FillNumSitesOfPat (void)
{
    ...
    if (memAllocs[ALLOC_NUMSITESOFPAT] == NO)
        {
        YvyraPrint ("%s   numSitesOfPat is not allocated\n", spacer);
        goto errorExit;
        }
    memAllocs[ALLOC_NUMSITESOFPAT] = NO;
    numSitesOfPat = (CLFlt *) SafeRealloc((void *) numSitesOfPat, numCompressedChars * chainParams.numChains * sizeof(YFlt));
    if (!numSitesOfPat)
        {
        YvyraPrint ("%s   Problem reallocating numSitesOfPat (%d)\n", spacer, numCompressedChars * chainParams.numChains * sizeof(YFlt));
        goto errorExit;
        }
    ...
    if (chainParams.numChains > 1)
        ...
        if (chainParams.weightScheme[0] + chainParams.weightScheme[1] > 0.0001)
            YvyraPrint ("%s   Reweighting of characters for chains 1 to %d\n", spacer, chainParams.numChains);

        /* check that we don't have an HMM */
        if (chainHasAdgamma == YES && chainParams.weightScheme[0] + chainParams.weightScheme[1] > 0.0001)
            {
            YvyraPrint ("%s   Reweighting of characters is not allowed with an autocorrelated gamma model\n", spacer);
            goto errorExit;
            }
        ...
        if (chainParams.weightScheme[0] + chainParams.weightScheme[1] > 0.0001)
            {
            YvyraPrint ("%s      Decreasing weight of %d characters\n", spacer, nToDecrease);
            YvyraPrint ("%s      Increasing weight of %d characters\n", spacer, nToIncrease);
            }
        ...
        for (j=1; j<chainParams.numChains; j++)
            ...
            for (i=0; i<nToDecrease; i++)
                ...
                if (numSitesOfPat[j * numCompressedChars + whichToChange] < 0)
                    {
                    YvyraPrint ("%s   Problem reweighting characters\n", spacer);
                    goto errorExit;
                    }
                }
...
#   if 0
    ...
    for (i=0; i<numCompressedChars; i++)
        ...
        for (j=0; j<chainParams.numChains; j++)
            {
            YvyraPrint ("%4.1lf ", numSitesOfPat[j * numCompressedChars + i]);
            }
        YvyraPrint ("\n");
        }
#   endif
...
#   endif
    ...
    /* ALLOC_BEST removed: species tree not supported in yvyra */
}
...
YFlt GibbsSampleGamma (int chain, int division, RandLong *seed)
{
    ...
    YFlt          ran, lnL, *bsVals, deltaLnL, temp;
    ModelInfo       *m;
    Tree            *t;
    TreeNode        *p;
...
int InitAdGamma (void)
{
    ...
    YvyraPrint ("%s   Initializing autocorrelated discrete gamma model\n", spacer);
    ...
    if (memAllocs[ALLOC_SITEJUMP] == YES)
        {
        YvyraPrint ("%s   siteJump not free in InitAdGamma\n", spacer);
        free (corrModel);
        return ERROR;
        }
    ...
    if (siteJump)
        ...
        {
        YvyraPrint ("%s   Problem allocating siteJump in InitAdGamma (%d ints)\n", spacer, numChar);
        free (corrModel);
        return ERROR;
        }
    ...
    for (i=1; i<=k; i++)
        ...
        if (j == 0)
            {
            YvyraPrint ("%s   ERROR: HMM model %d is empty.\n",spacer,i);
            free (corrModel);
            return (ERROR);
            }
        }
    ...
    if (memAllocs[ALLOC_MARKOVTIS] == YES)
        {
        YvyraPrint ("%s   markovTis not free in InitAdGamma\n", spacer);
        free (corrModel);
        return ERROR;
        }
    ...
    if (i >= MAX_SMALL_JUMP && markovTiN)
        ...
        {
        YvyraPrint ("%s   Problem allocating MarkovTis in InitAdGamma (%d YFlt)\n", spacer, 2 * MAX_RATE_CATS * MAX_RATE_CATS);
        for (i=0; i<MAX_SMALL_JUMP; i++)
            if (markovTi[i] != NULL) 
                FreeSquareDoubleMatrix (markovTi[i]);
        if (markovTiN != NULL) 
    ...
    if (memAllocs[ALLOC_RATEPROBS] == YES)
        {
        YvyraPrint ("%s   Space for rate probs not free in InitAdGamma\n", spacer);
        free (corrModel);
        return ERROR;
        }
    rateProbSpace = (YFlt *) SafeMalloc (2 * numLocalChains * rateProbRowSize * sizeof(YFlt));
    rateProbs = (YFlt **) SafeMalloc (numLocalChains * sizeof(YFlt *));
    if (!rateProbSpace || !rateProbs)
        {
        YvyraPrint ("%s   Problem allocating rate probs\n", spacer);
        if (rateProbSpace) 
            free (rateProbSpace);
        if (rateProbs) 
            free (rateProbs);
...
int InitAugmentedModels (void)
{
    ...
    YvyraPrint ("%s   Initializing variables for model augmentation\n", spacer);
...
#   endif
    ...
    for (d=0; d<numCurrentDivisions; d++)
        ...
        YvyraPrint ("%s   Division %d has %d unique site patterns\n", spacer, d+1, m->numChars);
...
#   endif
        */
        ...
        if (m->dataType == STANDARD)
            ...
            if (m->stateFreq->paramId == SYMPI_EQUAL)
                ...
                /* add space for USERTYPE characters */
                for (c=0; c<m->numChars; c++)
                    {
    ...
    /* set up likelihood calculators */
    if (condLikesUsed == YES)
        {
        YvyraPrint ("%s   Initializing conditional likelihoods\n", spacer);
...
#   endif
        ...
        if (m->useVec == VEC_NONE)
            YvyraPrint ("%s   Using standard non-SSE likelihood calculator for division %d (%s-precision)\n", spacer, d+1, (sizeof(CLFlt) == 4 ? "single" : "double"));
        else if (m->useVec == VEC_SSE)
            YvyraPrint ("%s   Using standard SSE likelihood calculator for division %d (single-precision)\n", spacer, d+1);
        else if (m->useVec == VEC_AVX)
            YvyraPrint ("%s   Using standard AVX likelihood calculator for division %d (single-precision)\n", spacer, d+1);
        else if (m->useVec == VEC_FMA)
            YvyraPrint ("%s   Using standard FMA likelihood calculator for division %d (single-precision)\n", spacer, d+1);
        else
            {
            YvyraPrint ("%s   WARNING! Using unknown SIMD likelihood calculator for division %d (single-precision)\n", spacer, d+1);
            return (ERROR);
            }
...
#   endif
        ...
        /* allocate eigen system space */
        if (m->nCijkParts > 0)
            {
            m->cijks = (YFlt**) SafeMalloc((numLocalChains + 1) * sizeof(YFlt*));
            if (!m->cijks)
                return ERROR;
            for (i=0; i<numLocalChains+1; i++)
                {
                m->cijks[i] = (YFlt*) SafeMalloc(m->cijkLength * sizeof(YFlt));
                if (!m->cijks[i])
                    return (ERROR);
                }
            }
                        ...
                        for (k=0; k<numReps; k++)
                            ...
                            for (s=0; s<m->nStates[c]; s++)
                                ...
                                if (IsBitSet(s, charBits))
                                    {
                                    if (hasTipWeights == YES && tipWeights != NULL)
                                        {
                                        int origC = origChar[m->compCharStart + c];
                                        YFlt w = tipWeights[(i * numChar + origC) * MAX_STD_STATES + s];
                                        (*cL) = (w > 0.0) ? (CLFlt)w : 1.0;
                                        }
                                    else
                                        (*cL) = 1.0;
                                    }
...
#   endif
    ...
    if (j > 0) /* don't bother allocating precalculated likelihoods if we only have parsimony model or morphological characters */
        ...
        if (memAllocs[ALLOC_PRELIKES] == YES)
            {
            YvyraPrint ("%s   Space for preLikes not free in InitChainCondLikes\n", spacer);
            return ERROR;
            }
        preLikeL = (CLFlt *) SafeMalloc (3 * j * sizeof(CLFlt));
        if (!preLikeL)
            {
            YvyraPrint ("%s   Problem allocating preLikes\n", spacer);
            return ERROR;
            }
...
int InitEigenSystemInfo (ModelInfo *m)
{
    ...
    /* Molecular datatypes not supported in yvyra */
    YvyraPrint ("%s   ERROR: Only STANDARD datatype supported in yvyra\n", spacer);
    return ERROR;
}
...
#   endif
    ...
    YvyraPrint ("%s   Initializing invariable-site conditional likelihoods\n", spacer);
...
int InitParsSets (void)
{
    ...
    for (d=0; d<numCurrentDivisions; d++)
        ...
        /* Doublet/Codon/Protein parsimony compression removed: molecular models not supported */
        else
            {
            YvyraPrint ("%s   Unrecognized data format during bitset compression\n", spacer);
            return ERROR;
            }
        }
...
int InitPrintParams (void)
{
    ...
    if (!printParam || !topologyPrintIndex)
        ...
        free (printParam);
        free (topologyPrintIndex);
        YvyraPrint ("%s   Could not allocate printParam vector in InitPrintParams\n", spacer);
        return (ERROR);
        }
...
{
    YFlt          oldLnPrior, newLnPrior, theta, N, growth, clockRate, sF, *sR, *eR, *fR;
    char            *sS;
    Model           *mp;
    ModelInfo       *m;
    ...
    if (!strcmp(mp->clockPr,"Coalescence"))
        ...
        if (LnCoalescencePriorPr (oldTree, &oldLnPrior, theta, growth) == ERROR)
            {
            YvyraPrint ("%s   Problem calculating prior for coalescence process\n", spacer);
            return (ERROR);
            }
        ...
        if (LnBirthDeathPriorPr (oldTree, clockRate, &oldLnPrior, *sR, *eR, sS, sF) == ERROR)
            {
            YvyraPrint ("%s   Problem calculating prior for birth-death process\n", spacer);
            return (ERROR);
            }
        ...
        if (LnFossilizationPriorPr (oldTree, clockRate, &oldLnPrior, sR, eR, fR, sF, sS) == ERROR)
            {
            YvyraPrint ("%s   Problem calculating prior for fossilized birth-death process\n", spacer);
            return (ERROR);
            }
...
#   endif
            ...
            /* CalcLikeAdgamma removed: adgamma correlation not supported in yvyra */
            if (0 && m->upDateCl == YES && m->correlation != NULL && m->mark != YES)
                {
...
YFlt LogDirPrior (Tree *t, ModelParams *mp, int PV)
{
    ...
    YFlt lnprior = 0.0, tb[2] = {0,0}, treeL = 0.0;
    YFlt aT, bT, a, c;
    TreeNode  *p;
...
YFlt LogPrior (int chain)
{
    ...
    const int       *rateCat;
    YFlt          *st, *sst, lnPrior, sum, x=0.0, clockRate, theta, popSize, growth, *alphaDir, newProp[190],
...
#   endif
        ...
        /* P_SPECIESTREE prior removed: species tree not supported in yvyra */
        }
...
int LnBirthDeathPriorPr (Tree *t, YFlt clockRate, YFlt *prob, YFlt sR, YFlt eR, char *sS, YFlt sF)
{
    if (!strcmp(sS, "Random")) 
        ...
        {
        YvyraPrint ("%s   ERROR: Sampling strategy for birth-death process not implemented\n", spacer);
        return (ERROR);
        }
}
...
int LnBirthDeathPriorPrRandom (Tree *t, YFlt clockRate, YFlt *prob, YFlt sR, YFlt eR, YFlt sF)
{
    ...
    nt = (YFlt *)SafeMalloc((size_t)(t->nIntNodes) * sizeof(YFlt));
    if (!nt)
        {
...
{
    int             i, n, m;
    YFlt          *nt, lambda, mu, nt_min, tmrca, ln_p0_t1;
    TreeNode        *p;
    Model           *mp;
...
{
    int             i, n, m;
    YFlt          *nt, lambda, mu, nt_2, tmrca, ln_p0_t1;
    TreeNode        *p;
    Model           *mp;
...
/*
 ...
 |
 */
YFlt LnP0 (YFlt t, YFlt b, YFlt d)
{
    YFlt      p0t;
...
YFlt LnP0Subsample (YFlt t, YFlt b, YFlt d, YFlt f)
{
    YFlt      p0t;
...
/*
 ...
 |
 */
YFlt LnP1 (YFlt t, YFlt b, YFlt d)
{
    YFlt      p1t;
...
YFlt LnP1Subsample (YFlt t, YFlt b, YFlt d, YFlt f)
{
    YFlt      p1t;
...
int Slice_i (YFlt t, YFlt *t_sl, int n_sl)
{
    int i = 0;
    assert (t > 0.0 && n_sl >= 0);
 ...
 */
YFlt  LnQi_fossil (YFlt t, YFlt *t_sl, int n_sl, YFlt *c1, YFlt *c2)
{
    YFlt lnq;
 ...
 */
YFlt  LnPi_fossil (YFlt t, YFlt *t_sl, int n_sl, YFlt *c1, YFlt *c2, YFlt *lambda, YFlt *mu, YFlt *psi)
{
    YFlt other;
    int i = Slice_i (t, t_sl, n_sl);
...
int LnFossilizationPriorPr (Tree *t, YFlt clockRate, YFlt *prob, YFlt *sR, YFlt *eR, YFlt *fR, YFlt sF, char *sS)
{
    ...
    if (!strcmp(sS, "FossilTip"))
        ...
        {
        YvyraPrint ("%s   Sampling strategy %s for fossilized birth-death process not implemented\n", spacer, sS);
        return (ERROR);
        }
    ...
    t_sl = (YFlt *)SafeMalloc(max_intv * sizeof(YFlt));
    if (!t_sl)
        {
        YvyraPrint ("%s   ERROR: Problem allocating t_sl in FossilizedBirthDeathPrior\n", spacer);
        return (ERROR);
        }
    n_sl = GetTotalRateShifts(mp, t_sl);  // number of total slices

    /* alloc memory for the parameters */
    lambda = (YFlt *)SafeMalloc((size_t)(n_sl +2) * sizeof(YFlt));
    mu     = (YFlt *)SafeMalloc((size_t)(n_sl +2) * sizeof(YFlt));
    psi    = (YFlt *)SafeMalloc((size_t)(n_sl +2) * sizeof(YFlt));
    rho    = (YFlt *)SafeMalloc((size_t)(n_sl +2) * sizeof(YFlt));
    if (!lambda || !mu || !psi || !rho)
        {
        YvyraPrint ("%s   ERROR: Problem allocating memory in FossilizedBirthDeathPrior\n", spacer);
        return (ERROR);
        }
...
int LnFossilizedBDPriorFossilTip (Tree *t, YFlt clockRate, YFlt *prob, YFlt *sR, YFlt *eR, YFlt *fR, YFlt sF)
{
    ...
    int         i;
    YFlt      x, lambda, mu, rho, psi, tmrca, c1, c2;
    TreeNode    *p;
    Model       *mp;
...
int GetTotalRateShifts (Model *mp, YFlt *shiftTimes)
{
    ...
    SortYFlt_Des (shiftTimes, 0, sTotal-1);
...
/*---------------------------------------------------------------------------------
 ...
 |
 ---------------------------------------------------------------------------------*/
int LnFossilizedBDPriorRandom (Tree *t, YFlt clockRate, YFlt *prob,
                               YFlt *lambda, YFlt *mu, YFlt *psi, YFlt *rho, YFlt *t_sl, int n_sl)
{
    /* Fossils are sampled with piecewise constant rates in the past.
       Extant taxa are sampled uniformly at random at present. */
    
    int         i, j,  M, E;
    YFlt      x, tmrca, *c1, *c2, *p_t;
    TreeNode    *p;
    Model       *mp;
    ...
    c1  = (YFlt *)SafeMalloc((size_t)(n_sl +1) * sizeof(YFlt));
    c2  = (YFlt *)SafeMalloc((size_t)(n_sl +1) * sizeof(YFlt));
    p_t = (YFlt *)SafeMalloc((size_t)(n_sl +1) * sizeof(YFlt));
    if (!c1 || !c2 || !p_t)
        {
        YvyraPrint ("%s   ERROR: Problem allocating memory in FossilizedBirthDeathPrior\n", spacer);
        return (ERROR);
...
/*---------------------------------------------------------------------------------
 ...
 |
 ---------------------------------------------------------------------------------*/
int LnFossilizedBDPriorDiversity (Tree *t, YFlt clockRate, YFlt *prob,
                                  YFlt *lambda, YFlt *mu, YFlt *psi, YFlt *rho, YFlt *t_sl, int n_sl)
{
    /* Fossils are sampled with piecewise constant rates in the past.
...
|
---------------------------------------------------------------------------------*/
int LnCoalescencePriorPr_Contemp (Tree *t, YFlt *prob, YFlt theta, YFlt growth)
{
    int             i, j, k, nNodes;
    YFlt          *ct, tempD, lastCoalescenceTime, coalescenceTime, intervalLength;
    TreeNode        *p;

    /* allocate space for the coalescence times */
    ct = (YFlt *)SafeMalloc((size_t)(t->nIntNodes) * sizeof(YFlt));
    if (!ct)
        {
...
int LnCoalescencePriorPr (Tree *t, YFlt *prob, YFlt theta, YFlt growth)
{
    ...
    ct = (YFlt *)SafeMalloc((size_t)nTimes * sizeof(YFlt));
    /* allocate space for the number of lineages */
    k = (int *)SafeMalloc((size_t)nTimes * sizeof(int));
    if (!ct || !k)
        {
        YvyraPrint ("%s   ERROR: Problem allocating coalescenct times\n", spacer);
        return (ERROR);
        }
...
YFlt LnUniformPriorPr (Tree *t, YFlt clockRate)
{
    ...
    YFlt      lnProb, treeAge, *nodeDepths=NULL;
    TreeNode    *p, *root;
    Model       *mp;
    ...
    for (i=0; i<t->nIntNodes; i++)
        ...
        nodeDepths = (YFlt *) SafeRealloc ((void *)nodeDepths, (nDatedTips+1)*sizeof(YFlt));
        ...
        SortYFlt_Asc (nodeDepths, 0, nDatedTips);   /* use index of left and right in call */
...
int NewtonRaphsonBrlen (Tree *t, TreeNode *p, int chain)
{
    int         c, i, j, s, k, n, d, division, nIterations, maxNumIterations,
                index, *rateCat, r;
    YFlt      vOld, *pi, *cijk, *eigenValues, *catRate, baseRate, length, theRate, tolerance,
...
FILE *OpenNewMBPrintFile (char *fileName)
{
    ...
    if (noWarn == YES)
        ...
        if ((fp = TestOpenTextFileR(fileName)) != NULL)
            ...
            if (autoOverwrite == NO)
                {
                YvyraPrint ("%s   Appending to file \"%s\"\n", spacer, fileName); 
                return (OpenTextFileA(fileName));
                }
            else 
                YvyraPrint ("%s   Overwriting file \"%s\"\n", spacer, fileName);      
            }
...
{
    int             c, j, k, nStates;
    YFlt          catLike, *like, *bs, *omegaCatFreq, *omega,
                    posProb, *ps, sum;
    CLFlt           **clP;
    ModelInfo       *m;
    ...
    like = (YFlt *) calloc (m->numOmegaCats, sizeof(YFlt));
    if (!clP || !like)
        {
        YvyraPrint ("%s   ERROR: Out of memory in PosSelProbs\n", spacer);
        free (clP);
        free (like);
        return (ERROR);
        }
...
int PosSelProbs_SSE (TreeNode *p, int division, int chain)
{
    ...
    if (!catLike || !mCatLike)
        {
        YvyraPrint ("%s   ERROR: Out of memory in PosSelProbs_SSE\n", spacer);
        free (catLike);
        free (mCatLike);
        return (ERROR);
        }
...
int SiteOmegas (TreeNode *p, int division, int chain)
{
    ...
    if (!clP || !like)
        {
        YvyraPrint ("%s   ERROR: Out of memory in SiteOmegas\n", spacer);
        free (clP);
        free (like);
        return (ERROR);
        }
...
int SiteOmegas_SSE (TreeNode *p, int division, int chain)
{
    ...
    if (!catLike || !mCatLike)
        {
        YvyraPrint ("%s   ERROR: Out of memory in SiteOmegas_SSE\n", spacer);
        free (catLike);
        free (mCatLike);
        return (ERROR);
        }
...
int PreparePrintFiles (void)
{
    ...
    if (memAllocs[ALLOC_FILEPOINTERS] == YES)
        {
        YvyraPrint ("%s   File pointers already allocated in PreparePrintFiles\n", spacer);
        return ERROR;
        }
    fpMcmc = NULL;
    fpSS = NULL;
    ...
    if (fpParm == NULL)
        {
        YvyraPrint ("%s   Could not allocate fpParm in PreparePrintFiles\n", spacer);
        return ERROR;
        }
    fpSiteLnL = (FILE **) SafeCalloc (chainParams.numRuns, sizeof (FILE *));
    if (fpSiteLnL == NULL)
        {
        YvyraPrint ("%s   Could not allocate fpSiteLnL in PreparePrintFiles\n", spacer);
        return ERROR;
        }
    ...
    if (fpTree == NULL)
        {
        YvyraPrint ("%s   Could not allocate fpTree in PreparePrintFiles\n", spacer);
        return ERROR;
        }
    fpTree[0] = (FILE **) SafeCalloc (numTrees*chainParams.numRuns, sizeof (FILE *));
    if (fpTree[0] == NULL)
        {
        YvyraPrint ("%s   Could not allocate fpTree[0] in PreparePrintFiles\n", spacer);
        return ERROR;
        }
    ...
    if (noWarn == NO)
        ...
        if (previousResults == YES)
            {
            YvyraPrint("\n");
            YvyraPrint("%s   There are results from a previous run saved using the same filename(s).\n", spacer);
            if (WantTo("Do you want to overwrite these results") == YES)
                ...
                {
                YvyraPrint("\n");
                YvyraPrint("%s   Please specify a different file name before running the MCMC analysis.\n", spacer);
                YvyraPrint("%s      You can do that using 'Mcmc filename=<name>'. You can also move or\n", spacer);
                YvyraPrint("%s      rename the old result files.\n", spacer);
                return ERROR;
                }
            }
        }
    ...
    for (n=0; n<chainParams.numRuns; n++)
        ...
        /* Open .slk file for per-site log-likelihoods */
        if (inferSiteLikes == YES)
            {
            if (chainParams.numRuns == 1)
                sprintf (fileName, "%s.slk", localFileName);
            else
                sprintf (fileName, "%s.run%d.slk", localFileName, n+1);
            if ((fpSiteLnL[n] = OpenNewMBPrintFile (fileName)) == NULL)
                {
                noWarn = oldNoWarn;
                autoOverwrite = oldAutoOverwrite;
                return (ERROR);
...
int PrintAncStates_Std (TreeNode *p, int division, int chain)
{
    ...
    for (c=0; c<numChar; c++)
        ...
        for (i=0; i<m->nStates[k]; i++)
            {
            SafeSprintf (&tempStr, &tempStrSize, "\t%s", PrintNum(cL[i]));
...
int PrintCheckPoint (long long gen)
{
    ...
    if (nErrors == 0)
        ...
        if ((fp = OpenTextFileW (ckpFileName)) == NULL)
            {
            YvyraPrint ("%s   Problem opening checkpoint file\n", spacer);
            nErrors++;
            }
        }
    ...
    /* write file header */
    YvyraPrintf (fp, "#NEXUS\n[ID: %s]\n[generation: %lli]\n", stamp, gen);
    YvyraPrintf (fp, "[seed: %lli, swapseed: %lli]\n", globalSeed, swapSeed);
    
    if (chainParams.isSS == YES)
        {
        /* dump to .ckp file current step contribution */
        YvyraPrintf (fp, "[SsAcumulators:");
#       if defined (MPI_ENABLED)
        for (j=0; j<chainParams.numRuns ; j++)
            {
            ...
            if (ierror != MPI_SUCCESS)
                {
                YvyraPrint ("%s   Problem with MPI_Reduce\n", spacer);
                return ERROR;
                }
            if (proc_id == 0)
                {
                YvyraPrintf (fp, " %.4f", sum);
                }
            }
#       else
        for (j=0; j<chainParams.numRuns ; j++)
            {
            YvyraPrintf (fp, " %.4f", log (stepAcumulatorSS[j]) + stepScalerSS[j]);
            }
#       endif
        YvyraPrintf (fp, "]\n");
        }
...
#   if defined (MPI_ENABLED)
    ...
    if (sumErrors > 0)
        {
        YvyraPrint ("%s   Memory allocation error on at least one processor\n", spacer);
        free (tempString);
        return ERROR;
        }
#   else
    ...
    for (i=0; i<numParams; i++)
        ...
        for (j=0; j<numLocalChains; j++)
            ...
            if (t->isRooted == YES && t->isClock == NO)
                ...
                SafeSprintf (&tempString, &tempStrSize, " = [&R] [&clockrate = %s] ", PrintNum(clockRate));
                }
        ...
        YvyraPrintf (fp, "%s", printString);
#if defined (MPI_ENABLED)
        MPI_Allreduce (&nErrors, &sumErrors, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
        if (sumErrors > 0)
            {
            YvyraPrint ("%s   Print error on at least one processor\n", spacer);
            goto errorExit;
            }
...
if (proc_id == 0)
    ...
    YvyraPrintf (fp,"begin yvyra;\n");
    YvyraPrintf (fp, "\tstartvals\n");
...
int PrintMCMCDiagnosticsToFile (long long curGen)
{
    ...
    if (curGen == 0)
        {
        // YvyraPrintf (fpMcmc, "[LEGEND:]\n");
        YvyraPrintf (fpMcmc, "[ID: %s]\n", stamp);
        YvyraPrintf (fpMcmc, "[   Gen                --  Generation]\n");
        if (chainParams.allChains == YES)
            YvyraPrintf (fpMcmc, "[   <name1>(<name2>)$acc_run2_chn3 --  Acceptance rate of move <name1> changing parameter <name2> in run 2, chain 3]\n");
        else /* if (chainParams.allChains == NO) */
            YvyraPrintf (fpMcmc, "[   <name1>(<name2>)$acc_run2      --  Acceptance rate of move <name1> changing parameter 1 in run 2]\n");
        if (chainParams.numChains > 1 && chainParams.numRuns > 1)
            YvyraPrintf (fpMcmc, "[   Swap(1<>2)$acc_run3            --  Acceptance rate of swaps between chains 1 and 2 in run 3]\n");
        else if (chainParams.numChains > 1 && chainParams.numRuns == 1)
            YvyraPrintf (fpMcmc, "[   Swap(1<>2)$acc                 --  Acceptance rate of swaps between chains 1 and 2]\n");
        if (chainParams.diagnStat == AVGSTDDEV)
            diagnstat = "Average";
        else
            diagnstat = "Maximum";
        if (chainParams.numRuns > 1 && numTopologies == 1 && chainParams.allComps == NO)
            YvyraPrintf (fpMcmc, "[   StdDev(s)                      --  %s standard deviation of split frequencies]\n", diagnstat);
        else if (chainParams.numRuns > 1 && numTopologies == 1 && chainParams.allComps == YES)
            YvyraPrintf (fpMcmc, "[   StdDev(s)(run1-run2)           --  %s standard deviation of split frequencies between runs 1 and 2]\n", diagnstat);
        else if (chainParams.numRuns > 1 && numTopologies > 1 && chainParams.allComps == NO)
            YvyraPrintf (fpMcmc, "[   StdDev(s.tree1)                --  %s standard deviation of split frequencies for topology 1]\n", diagnstat);
        else if (chainParams.numRuns > 1 && numTopologies > 1 && chainParams.allComps == YES)
            YvyraPrintf (fpMcmc, "[   StdDev(s.tree1)(run1-run2)     --  %s standard deviation of split frequencies between runs 1 and 2 for topology 1]\n", diagnstat);

        YvyraPrintf (fpMcmc, "Gen");
        for (n=0; n<chainParams.numRuns; n++)
            {
            if (chainParams.allChains == YES)
                {
                for (i=0; i<chainParams.numChains; i++)
                    ...
                    for (j=0; j<numUsedMoves; j++)
                        {
                        theMove = usedMoves[j];
                        YvyraPrintf (fpMcmc, "\t%s(%s)$acc_run%d_chn%d", theMove->moveType->shortName,
                                theMove->parm->name, n+1, i+1);
                        if (theMove->moveType->Autotune != NULL && chainParams.autotune == YES)
                            YvyraPrintf (fpMcmc, "\t%s(%s)$%s_run%d_chn%d", theMove->moveType->shortName,
                ...
                for (i=0; i<numUsedMoves; i++)
                    ...
                    if (chainParams.numRuns == 1)
                        {
                        YvyraPrintf (fpMcmc, "\t%s(%s)$acc", theMove->moveType->shortName, theMove->parm->name);
                        if (theMove->moveType->Autotune != NULL && chainParams.autotune == YES)
                            YvyraPrintf (fpMcmc, "\t%s(%s)$%s", theMove->moveType->shortName, theMove->parm->name, theMove->moveType->shortTuningName[0]);
                        }
                    else
                        {
                        YvyraPrintf (fpMcmc, "\t%s(%s)$acc_run%d", theMove->moveType->shortName, theMove->parm->name, n+1);
                        if (theMove->moveType->Autotune != NULL && chainParams.autotune == YES)
                            YvyraPrintf (fpMcmc, "\t%s(%s)$%s_run%d", theMove->moveType->shortName, theMove->parm->name, theMove->moveType->shortTuningName[0], n+1);
                        }
                    }
                }
            if (chainParams.numChains > 1)
                ...
                for (i=0; i<chainParams.numChains; i++)
                    ...
                    for (j=i+1; j<chainParams.numChains; j++)
                        {
                        if (chainParams.numRuns == 1)
                            YvyraPrintf (fpMcmc, "\tSwap(%d<>%d)$acc", i+1, j+1);
                        else
                            YvyraPrintf (fpMcmc, "\tSwap(%d<>%d)$acc(%d)", i+1, j+1, n+1);
                        }
                    }
                }
            }
        ...
        if (chainParams.numRuns > 1)
            ...
            for (n=0; n<numTopologies; n++)
                ...
                if (numTopologies == 1)
                    {
                    if (chainParams.diagnStat == AVGSTDDEV)
                        YvyraPrintf (fpMcmc, "\tAvgStdDev(s)");
                    else
                        YvyraPrintf (fpMcmc, "\tMaxStdDev(s)");
                    }
                else
                    {
                    if (chainParams.diagnStat == AVGSTDDEV)
                        YvyraPrintf (fpMcmc, "\tAvgStdDev(s.tree%d)", n+1);
                    else
                        YvyraPrintf (fpMcmc, "\tMaxStdDev(s.tree%d)", n+1);
                    }
                    ...
                    for (i=0; i<chainParams.numRuns; i++)
                        ...
                        for (j=i+1; j<chainParams.numRuns; j++)
                            ...
                            if (numTopologies == 1)
                                {
                                if (chainParams.diagnStat == AVGSTDDEV)
                                    YvyraPrintf (fpMcmc, "\tAvgStdDev(s)(run%d_run%d)", i+1,j+1);
                                else
                                    YvyraPrintf (fpMcmc, "\tMaxStdDev(s)(run%d_run%d)", i+1, j+1);
                                }
                            else
                                {
                                if (chainParams.diagnStat == AVGSTDDEV)
                                    YvyraPrintf (fpMcmc, "\tAvgStdDev(s.tree%d)(run%d_run%d)", n+1, i+1, j+1);
                                else
                                    YvyraPrintf (fpMcmc, "\tMaxStdDev(s.tree%d)(run%d_run%d)", n+1, i+1, j+1);
                                }
                            }
                        }
                    }
    ...
    for (n=0; n<chainParams.numRuns; n++)
            ...
            for (j=n*chainParams.numChains; j<(n+1)*chainParams.numChains; j++)
                ...
                for (i=0; i<numUsedMoves; i++)
                    ...
                    if (theMove->nBatches[j] < 1)
                        YvyraPrintf (fpMcmc, "\tNA");
                    else
                        YvyraPrintf (fpMcmc, "\t%.6f", theMove->lastAcceptanceRate[j]);
                    if (theMove->moveType->Autotune != NULL && chainParams.autotune == YES)
                        YvyraPrintf (fpMcmc, "\t%.6e", theMove->tuningParam[j][0]);
                    }
                }
            }
        else
        ...
        if (chainParams.numChains > 1)
            ...
            for (i=0; i<chainParams.numChains; i++)
                ...
                for (j=i+1; j<chainParams.numChains; j++)
                    {
                    YvyraPrintf (fpMcmc, "\t%.6f", (YFlt) swapInfo[n][i][j] / (YFlt) swapInfo[n][j][i]);
                    }
                }
            }
        }
    ...
    if (chainParams.numRuns > 1)
        ...
        for (n=0; n<numTopologies; n++)
            ...
            if (chainParams.relativeBurnin == NO && curGen < (long long)(chainParams.chainBurnIn) * chainParams.sampleFreq)
                ...
                {
                if (chainParams.diagnStat == AVGSTDDEV)
                    YvyraPrintf (fpMcmc, "\t%.6f", chainParams.stat[n].avgStdDev);
                else
                    YvyraPrintf (fpMcmc, "\t%.6f", chainParams.stat[n].max);
                }
            if (chainParams.allComps == YES)
                {
                for (i=0; i<chainParams.numRuns; i++)
                    ...
                    for (j=i+1; j<chainParams.numRuns; j++)
                        ...
                        if (chainParams.relativeBurnin == NO && curGen < (long long)(chainParams.chainBurnIn) * chainParams.sampleFreq)
                            ...
                            YvyraPrintf (fpMcmc, "\t%.6f", chainParams.stat[n].pair[i][j] / chainParams.stat[n].pair[j][i]);
                        else /*if (chainParams.diagnStat == MAXSTDDEV) */
                            YvyraPrintf (fpMcmc, "\t%.6f", chainParams.stat[n].pair[i][j]);
                        }
                    }
                }
            }
...
int PrintMPISlaves (FILE *fp)
{
    ...
    for (i=1; i<num_procs; i++)
        ...
        if (proc_id == 0)
            ...
            if (ierror != MPI_SUCCESS)
                {
                YvyraPrint ("%s   Problem receiving string length from proc_id = %d\n", spacer, i);
                nErrors++;
                }
            }
        else if (proc_id == i)
            ...
            if (ierror != MPI_SUCCESS)
                {
                YvyraPrint ("%s   Problem sending string length from proc_id = %d\n", spacer, i);
                nErrors++;
                }
            }
        ...
        if (nErrors == 0 && proc_id == 0 && len+5 > strlen(s))
            ...
            if ((s = (char *) SafeRealloc ((void *)s, ((size_t)len+5)*sizeof(char))) == NULL)
                {
                YvyraPrint ("%s   Problem reallocating %d chars to string 's' on proc 0 in PrintMPISlaves()\n", spacer, len+5);
                nErrors++;
                }
            }
        ...
        if (nErrors == 0)
            ...
            if (proc_id == 0)
                ...
                if (ierror != MPI_SUCCESS)
                    {
                    YvyraPrint ("%s   Problem receiving printString from proc_id = %d\n", spacer, i);
                    nErrors++;
                    }
                /* print string */
                if (nErrors == 0)
                ...
                if (ierror != MPI_SUCCESS)
                    {
                    YvyraPrint ("%s   Problem sending printString from proc_id = %d\n", spacer, i);
                    nErrors++;
                    }
                }
            }
    ...
    if (sumErrors > 0)
        {
        YvyraPrint ("%s   Problem with the printing in PrintMPISlaves().\n", spacer);
        return (ERROR);
        }
...
int PrintParsMatrix (void)
{
    ...
    for (d=0; d<numCurrentDivisions; d++)
        ...
        for (c=0; c<m->numChars; c++)
            {
            YvyraPrint ("Parsimony sets for character %d -- \n", (c / m->nParsIntsPerSite));
            for (i=0; i<numTaxa; i++)
                {
...
{
    int             c, j, k, nStates;
    YFlt          siteLike, catLike, *bs, *catRate, baseRate;
    CLFlt           *clP, *siteRates;
    char            *tempStr;
    int             tempStrSize = TEMPSTRSIZE;
    ModelInfo       *m;
    ...
    for (c=0; c<numChar; c++)
        ...
        SafeSprintf (&tempStr, &tempStrSize, "\t%s", PrintNum(siteRates[j]));
        if (AddToPrintString (tempStr) == ERROR) return (ERROR);
        }
...
int PrintStates (long long curGen, int coldId)
{
    ...
    if (!printString)
        {
        YvyraPrint ("%s   Problem allocating printString (%d)\n", spacer, printStringSize * sizeof(char));
        goto errorExit;
        }
    if (!tempStr)
        {
    ...
    *tempStr = '\0';
    ...
    if (inferPosSel == YES || inferSiteOmegas == YES)
        ...
        if (memAllocs[ALLOC_POSSELPROBS] == YES)
            {
            YvyraPrint ("%s   posSelProbs not free in PrintStates\n", spacer);
            goto errorExit;
            }
        posSelProbs = (YFlt *)SafeMalloc((size_t)numCompressedChars * sizeof(YFlt));
        if (!posSelProbs)
            {
            YvyraPrint ("%s   Problem allocating posSelProbs (%d)\n", spacer, numCompressedChars * sizeof(YFlt));
            goto errorExit;
            }
        for (i=0; i<numCompressedChars; i++)
            posSelProbs[i] =  -10.0;
    ...
    if (inferPosSel == YES || inferSiteOmegas == YES || inferSiteRates == YES || inferAncStates == YES)
        ...
        if (!printedChar)
            {
            YvyraPrint ("%s   Problem allocating printedChar (%d)\n", spacer, numChar * sizeof(int));
            goto errorExit;
            }
        for (i=0; i<numChar; i++)
            printedChar[i] = NO;
    ...
    if (curGen == 0)
        ...
        /* per-site log-likelihoods now written to separate .slk file */

        /* estimated character weight headers added in PrintStatesToFiles */
            ...
            for (d=0; d<numCurrentDivisions; d++)
                ...
                for (i=0; i<numDefinedConstraints; i++)
                    ...
                    for (j=0; j<numChar; j++)
                        ...
                        /* Codon/Doublet/Protein print branches removed: molecular models not supported */
                        }
                    }
                }
            }
    ...
    if (AddToPrintString (tempStr) == ERROR) goto errorExit;
    SafeSprintf (&tempStr, &tempStrSize, "\t%s", PrintNum(curLnL[coldId]));
    if (AddToPrintString (tempStr) == ERROR) goto errorExit;
    SafeSprintf (&tempStr, &tempStrSize, "\t%s", PrintNum(curLnPr[coldId]));
    if (AddToPrintString (tempStr) == ERROR) goto errorExit;
    ...
    for (i=0; i<numParams; i++)
        ...
        if (p->paramType == P_BRLENS)
            ...
            if (tree->isClock == NO)
                {
                SafeSprintf (&tempStr, &tempStrSize, "\t%s", PrintNum(TreeLength(p, coldId)));
                if (AddToPrintString (tempStr) == ERROR) goto errorExit;
                }
            else
                {
                SafeSprintf (&tempStr, &tempStrSize, "\t%s", PrintNum(tree->root->left->nodeDepth));
                if (AddToPrintString (tempStr) == ERROR) goto errorExit;
                SafeSprintf (&tempStr, &tempStrSize, "\t%s", PrintNum(TreeLength(p, coldId)));
                if (AddToPrintString (tempStr) == ERROR) goto errorExit;
                }
            ...
            {
            /* print proportion of ancestral fossils */
            SafeSprintf (&tempStr, &tempStrSize, "\t%s", PrintNum(PropAncFossil(p, coldId)));
            if (AddToPrintString (tempStr) == ERROR) goto errorExit;
            }
        }
    ...
    for (i=0; i<numPrintParams; i++)
        ...
        if (p->paramId == SYMPI_EXP || p->paramId == SYMPI_UNI ||
            ...
            if (p->paramId != SYMPI_FIX_MS)
                {
                SafeSprintf (&tempStr, &tempStrSize, "\t%s", PrintNum(st[0]));
                if (AddToPrintString (tempStr) == ERROR) goto errorExit;
                }
            ...
            for (j=0; j<p->nSympi; j++)
                ...
                for (k=0; k<p->sympinStates[j]; k++)
                    {
                    SafeSprintf (&tempStr, &tempStrSize, "\t%s", PrintNum(*sst++));
                    if (AddToPrintString (tempStr) == ERROR) goto errorExit;
                    }
                }
            }
            ...
            for (j=0; j<p->nSubValues; j++)
                {
                SafeSprintf (&tempStr, &tempStrSize, "\t%s", PrintNum(sst[j]));
                if (AddToPrintString (tempStr) == ERROR) goto errorExit;
                }
            ...
            if (!strcmp(mp->ratemultFormat,"Ratio"))
                ...
                for (j=0; j<p->nValues; j++)
                    {
                    SafeSprintf (&tempStr, &tempStrSize, "\t%s", PrintNum(st[j] / st[0]));
                    if (AddToPrintString (tempStr) == ERROR) goto errorExit;
                    }
                }
                ...
                for (j=0; j<p->nValues; j++)
                    {
                    SafeSprintf (&tempStr, &tempStrSize, "\t%s", PrintNum(st[j] / sum));
                    if (AddToPrintString (tempStr) == ERROR) goto errorExit;
                    }
                }
            else
                ...
                for (j=0; j<p->nValues; j++)
                    {
                    SafeSprintf (&tempStr, &tempStrSize, "\t%s", PrintNum(st[j]));
                    if (AddToPrintString (tempStr) == ERROR) goto errorExit;
                    }
                }
            }
    ...
    if (inferPosSel == YES)
        ...
        for (i=0; i<numChar; i++)
            ...
            if (!(posSelProbs[compressedCharPosition] < 0.0) && printedChar[i] == NO && charInfo[i].isExcluded == NO)
                ...
                SafeSprintf (&tempStr, &tempStrSize, "\t%s", PrintNum(posSelProbs[compressedCharPosition]));
                if (AddToPrintString (tempStr) == ERROR) goto errorExit;
...
#   if !defined (MPI_ENABLED)
    ...
    for (chn=0; chn<numLocalChains; chn++)
        ...
        if ((chainId[chn] % chainParams.numChains) == 0)
            ...
            /* write estimated weight values to .p file as extra line */
            if (hasEstimatedWeights)
                {
                static int weightHeaderWritten = 0;
                if (!weightHeaderWritten && curGen == 0)
                    {
                    /* Write a separate weight tracking file */
                    char wFileName[300];
                    if (chainParams.numRuns == 1)
                        sprintf(wFileName, "%s.w", chainParams.chainFileName);
                    else
                        sprintf(wFileName, "%s.run%d.w", chainParams.chainFileName, runId + 1);
                    FILE *wfp = fopen(wFileName, "w");
                    if (wfp)
                        {
                        fprintf(wfp, "[ID: %ld]\nGen", (long int)stamp);
                        PrintCharWeightHeader(wfp);
                        fprintf(wfp, "\n");
                        fprintf(wfp, "%lld", curGen);
                        PrintCharWeightValues(wfp);
                        fprintf(wfp, "\n");
                        fclose(wfp);
                        }
                    weightHeaderWritten = 1;
                    }
                else
                    {
                    char wFileName[300];
            ...
            /* write per-site log-likelihoods to .slk file */
            if (inferSiteLikes == YES && fpSiteLnL && fpSiteLnL[runId])
                {
                int d;
                ModelInfo *m;
                ...
                if (curGen == 0)
                    ...
                    fprintf (fpSiteLnL[runId], "[ID: %ld]\n", (long int)stamp);

                    /* Column headers */
                    fprintf (fpSiteLnL[runId], "Gen");
                    for (d=0; d<numCurrentDivisions; d++)
                        {
                        m = &modelSettings[d];
                        if (m->printSiteLikes == YES && m->dataType == STANDARD)
                            {
                            int nc = m->numChars - m->numDummyChars;
                            for (j=0; j<nc; j++)
                                fprintf (fpSiteLnL[runId], "\tlnL(%d.%d)", d+1, j+1);
                            }
                        }
                    fprintf (fpSiteLnL[runId], "\n");

                    /* Pattern weights line */
                    fprintf (fpSiteLnL[runId], "[weights:");
                    for (d=0; d<numCurrentDivisions; d++)
                        {
                        m = &modelSettings[d];
                        if (m->printSiteLikes == YES && m->dataType == STANDARD)
                            {
                            int nc = m->numChars - m->numDummyChars;
                            nSitesOfPat = numSitesOfPat + (0 * numCompressedChars) + m->compCharStart;
                            for (j=0; j<nc; j++)
                                fprintf (fpSiteLnL[runId], "\t%.0f", (double)nSitesOfPat[m->numDummyChars + j]);
                            }
                        }
                    fprintf (fpSiteLnL[runId], "]\n");
                    }
                ...
                /* Data row */
                fprintf (fpSiteLnL[runId], "%lld", curGen);
                for (d=0; d<numCurrentDivisions; d++)
                    {
                    m = &modelSettings[d];
                    if (m->printSiteLikes == YES && m->siteLnL != NULL)
                        {
                        int nc = m->numChars - m->numDummyChars;
                        for (j=0; j<nc; j++)
                            fprintf (fpSiteLnL[runId], "\t%s", PrintNum(m->siteLnL[j]));
                        }
                    }
                fprintf (fpSiteLnL[runId], "\n");
                fflush (fpSiteLnL[runId]);
                }
...
#   else
    ...
    if (ierror != MPI_SUCCESS)
        {
        YvyraPrint ("%s   Problem at chain barrier.\n", spacer);
        return ERROR;
        }
    tag = nErrors = 0;
    ...
    for (runId=0; runId<chainParams.numRuns; runId++)
        ...
        if (ierror != MPI_SUCCESS)
            {
            YvyraPrint ("%s   Problem finding processor with chain to print.\n", spacer);
            return (ERROR);
            }
        ...
        if (sumErrors > 0)
            {
            YvyraPrint ("%s   Problem with PrintStates.\n", spacer);
            return ERROR;
            }
        ...
        if (proc_id == 0 || proc_id == procWithChain)
            ...
            if (procWithChain != 0)
                ...
                if (proc_id == procWithChain)
                    ...
                    if (ierror != MPI_SUCCESS)
                        {
                        YvyraPrint ("%s   Problem receiving printStringSize from proc_id = %d\n", spacer, procWithChain);
                        nErrors++;
                        }
                    printString = (char *)SafeMalloc((size_t)printStringSize * sizeof(char));
                    if (!printString)
        ...
        if (sumErrors > 0)
            {
            YvyraPrint ("%s   Problem with first communication (states).\n", spacer);
            return ERROR;
            }
        ...
        if (sumErrors > 0)
            {
            YvyraPrint ("%s   Problem with second communication (states).\n", spacer);
            return ERROR;
            }
        ...
        for (i=0; i<numPrintTreeParams; i++)
            ...
            if (sumErrors > 0)
                {
                YvyraPrint ("%s   Problem with printing trees.\n", spacer);
                return ERROR;
                }
...
#   endif
        ...
        for (i=0; i<chainParams.numChains; i++)
            ...
            for (j=0; j<chainParams.numChains; j++)
                ...
                if (i < j)
                    ...
                    if (swapInfo[n][j][i] <= 0)
                        {
                        YvyraPrint ("%*c%s ", maxLen-3, ' ', " NA ");
                        }
                    else
                        {
                        SafeSprintf (&tempStr, &tempStrSize, "%1.2lf", (YFlt)swapInfo[n][i][j]/swapInfo[n][j][i]);
                        len = (int) strlen(tempStr);
                        YvyraPrint ("%*c%1.2lf ", maxLen-len+1, ' ', (YFlt)swapInfo[n][i][j]/swapInfo[n][j][i]);
                        }
                    }
                else if (i == j)
                    {
                    YvyraPrint ("%*c ", maxLen+1, ' ');
                    }
                else
                    {
                    SafeSprintf (&tempStr, &tempStrSize, "%d", swapInfo[n][i][j]);
                    len = (int) strlen(tempStr);
                    YvyraPrint ("%*c%d ", maxLen-len+1, ' ', swapInfo[n][i][j]);
                    }
                }
            YvyraPrint ("\n");
            }
    ...
    YvyraPrint ("%s   Lower diagonal: Number of attempted state exchanges between chains\n", spacer);
        
    YvyraPrint ("\n%s   Chain information:\n\n", spacer);
    YvyraPrint ("%s     ID -- Heat ", spacer);
    if (reweightingChars == YES)
        YvyraPrint ("%% Dn %% Up\n");
    else
        YvyraPrint ("\n");
    ...
    for (i=0; i<chainParams.numChains; i++)
        {
        YvyraPrint ("%s   %4d -- %1.2lf ", spacer, i+1, Temperature (i)/*1.0 / (1.0 + chainParams.chainTemp * i)*/);
        if (reweightingChars == YES)
            {
            if (i == 0)
                {
                YvyraPrint ("  0%%   0%% (cold chain)\n");
                }
            else
                {
                SafeSprintf (&tempStr, &tempStrSize, "%d", (int)chainParams.weightScheme[0]);
                len = (int) strlen(tempStr);
                YvyraPrint ("%*c%d%% ", 3-len, ' ', (int)chainParams.weightScheme[0]);
    ...
    if (chainParams.userDefinedTemps == NO)
        {
        YvyraPrint ("\n%s   Heat = 1 / (1 + T * (ID - 1))\n", spacer);
        YvyraPrint ("%s      (where T = %1.2lf is the temperature and ID is the chain number)\n", spacer, chainParams.chainTemp);
        }
    if (reweightingChars == YES)
        YvyraPrint ("%s   Reweighting increment = %1.2lf\n", spacer, chainParams.weightScheme[2]);
    YvyraPrint ("\n");
        ...
        for (c=m->compCharStart; c<m->compCharStop; c=j)
            ...
            for (i=0; i<numTaxa; i++)
                ...
                for (nextColumn=13; nextColumn < printWidth; nextColumn+=nDigits + 1)
                    ...
                    if (j >= m->compCharStop)
                        break;
                    YvyraPrint ("%*d ",nDigits, m->termState[i][j-c]);
                    j++;
                    }
                YvyraPrint ("\n");
                }
...
#   endif
    ...
    for (n=0; n<numTopologies; n++)
        ...
        if (numTopologies == 1)
            {
            if (chainParams.diagnStat == AVGSTDDEV)
                YvyraPrint ("%s   Pairwise average standard deviation of split frequencies (upper triangle)\n", spacer);
            else
                YvyraPrint ("%s   Pairwise maximum standard deviation of split frequencies (upper triangle)\n", spacer);
            YvyraPrint ("%s      and number of qualifying splits for each comparison (lower triangle):\n\n", spacer);
            }
        else
            {
            if (chainParams.diagnStat == AVGSTDDEV)
                YvyraPrint ("%s   Pairwise average standard deviation of split frequencies in topology %d (upper triangle)\n", spacer, n);
            else
                YvyraPrint ("%s   Pairwise maximum standard deviation of split frequencies in topology %d (upper triangle)\n", spacer, n);
            YvyraPrint ("%s      and number of qualifying splits for each comparison (lower triangle):\n\n", spacer);
            }
        ...
        for (i=0; i<chainParams.numRuns; i++)
            ...
            for (j=0; j<chainParams.numRuns; j++)
                ...
                if (i < j)
                    {
                    if (chainParams.diagnStat == AVGSTDDEV)
                        SafeSprintf (&tempStr, &tempStrSize, "%1.3lf", (YFlt) (stat->pair[i][j]) / (YFlt) (stat->pair[j][i]));
                    else /* if (chainParams.diagnStat == MAXSTDDEV) */
                        SafeSprintf (&tempStr, &tempStrSize, "%1.3lf", stat->pair[i][j]);
                    len = (int) strlen(tempStr);
                    YvyraPrint ("%*c%s ", maxLen-len+1, ' ', tempStr);
                    }
                else if (i == j)
                    {
                    YvyraPrint ("%*c ", maxLen+1, ' ');
...
#   if defined (MPI_ENABLED)
    ...
    if (curGen == 0)
        ...
        if (chainParams.isSS == NO && chainParams.mcmcDiagn == YES && chainParams.numRuns > 1)
            ...
            YvyraPrint ("\n");
            if (chainParams.relativeBurnin == YES)
                YvyraPrint ("%s   Using a relative burnin of %.1f %% for diagnostics\n", spacer, 100.0*chainParams.burninFraction);
            else
                YvyraPrint ("%s   Using an absolute burnin of %d samples for diagnostics\n", spacer, chainParams.chainBurnIn);
            }
        YvyraPrint ("\n");
        YvyraPrint ("%s   Chain results (%lli generations requested):\n\n", spacer, chainParams.numGen);
        }
    YvyraPrint ("%s   %4lli -- ", spacer, curGen);
        ...
        if (i > chainParams.printMax)   
            ...
            if (i == chainParams.printMax +1)
                ...
                if (numLocalColdChains > 0 && numLocalColdChains > numFirstAndLastCold)
                    YvyraPrint ("[...%d more local chains...] ", numLocalChains - chainParams.printMax);
                else
                    YvyraPrint ("(...%d more local chains...) ", numLocalChains - chainParams.printMax);
                continue;
                }
    ...
    if (numLocalColdChains == chainParams.numRuns)
        YvyraPrint ("(...%d remote chains...) ", (chainParams.numChains*chainParams.numRuns) - numLocalChains);
    else
        YvyraPrint ("[...%d remote chains...] ", (chainParams.numChains*chainParams.numRuns) - numLocalChains);

    if (curGen > 0)
        {
        timePerGen = (YFlt) ((YFlt)(endingT-startingT)/(YFlt)(curGen-startGen));
        nSecs = (int)((chainParams.numGen - curGen) * timePerGen);
        nHours = nSecs / 3600;
        nSecs  = nSecs % 3600;
        nMins  = nSecs / 60; 
        nSecs  = nSecs % 60;
        YvyraPrint ("-- %d:%0.2d:%0.2d", nHours, nMins, nSecs);
        }
    YvyraPrint ("\n");
    fflush (stdout);
...
#   else
    ...
    YvyraPrint ("%s   %5lli -- ", spacer, curGen);
    if (numLocalChains == 1)
        YvyraPrint ("%1.3lf ", curLnL[0]);
    else
        {
        i = 0;
        for (chn=0; chn<numLocalChains; chn++)
            ...
            if (chainParams.numChains == 1)
                ...
                if (chainParams.printAll == YES)
                    ...
                    YvyraPrint ("[%1.3lf][%d] .. ", curLnL[chn], chn % chainParams.numChains + 1);
                i++;
                }
            else if (chainParams.printAll == YES)
                {
...
int PrintTree (long long curGen, Param *treeParam, int chain, int showBrlens, YFlt clockRate)
{
    ...
    *printString = '\0';
    ...
    if (tree->isRooted == YES && tree->isCalibrated == NO)
        ...
        SafeSprintf (&tempStr, &tempStrSize, " = [&R] [&clockrate=%s] ", PrintNum(clockRate));
    ...
    for (n=0; n<numGlobalChains; n++)
        ...
        for (i=0; i<numUsedMoves; i++)
            ...
            if (proc_id == 0)
                ...
                mv->lastAcceptanceRate[n] = (YFlt)sum[5];
                if (mv->moveType->Autotune != NULL)
                    mv->tuningParam[n][0]=(YFlt)sum[6];
                }
            }
        }
...
int ReassembleParamVals (int *curId)
{
    ...
    YFlt          *x, *brlens, **position, **rateMult;
    MPI_Status      status;
    MPI_Request     request;
    Tree            *tree;
    Param           *p;

    extern YFlt   *paramValues;
    ...
    for (proc=1; proc<num_procs; proc++, lower=upper)
        ...
        /* mcmc trees */
        brlens = (YFlt *) SafeCalloc (2*numLocalTaxa, sizeof(YFlt));
                ...
                if (p->paramType == P_CPPEVENTS)
                    ...
                    if (proc_id == proc)
                        ...
                        for (k=0; k<2*numLocalTaxa; k++)
                            ...
                            if (nEvents[k] > 0)
                                {
                                position[k] = (YFlt *) SafeCalloc (nEvents[k], sizeof(YFlt));
                                rateMult[k] = (YFlt *) SafeCalloc (nEvents[k], sizeof(YFlt));
...
int ReassembleTuningParams (void)
{
    ...
    x = (YFlt *) SafeCalloc (2*numUsedMoves, sizeof(YFlt));
    sum = x + numUsedMoves;
...
int RemoveTreeFromPartitionCounters (Tree *tree, int treeId, int runId)
{
    ...
    for (i=0; i<tree->nIntNodes-1; i++)
        ...
        if ((RemovePartition (partFreqTreeRoot[treeId], partition[p->index], runId)) == ERROR)
            {
            YvyraPrint ("%s   Could not remove partition %d in RemoveTreeFromPartitionCounters\n", spacer, p->index);
...
int ConfirmAbortRun(void)
{
    ...
    YvyraPrint ("   Do you really want to stop the run (y/n)?");
...
void ResetChainIds (void)
{
    ...
    Param   *p;
    YFlt  *fromVals, *toVals, *swapVals, **fromPosition, **toPosition,
        ...
        for (j=0; j<numParams; j++)
            ...
            if (p->paramType == P_CPPEVENTS)
                ...
                for (k=0; k<2*numLocalTaxa; k++)
                    ...
                    if (from[k] > 0)
                        {
                        toPosition[k] = (YFlt *) SafeRealloc ((void *)toPosition[k], from[k]*sizeof (YFlt));
                        toRateMult[k] = (YFlt *) SafeRealloc ((void *)toRateMult[k], from[k]*sizeof (YFlt));
                    ...
                    if (swap[k] > 0)
                        {
                        fromPosition[k] = (YFlt *) SafeRealloc ((void *)fromPosition[k], swap[k]*sizeof (YFlt));
                        fromRateMult[k] = (YFlt *) SafeRealloc ((void *)fromRateMult[k], swap[k]*sizeof (YFlt));
...
#   endif
    ...
    if (memAllocs[ALLOC_FILEPOINTERS] == YES)
        {
        YvyraPrint ("%s   File pointers already allocated in ReusePreviousResults\n", spacer);
        return ERROR;
        }
    fpMcmc = NULL;
    fpSS = NULL;
    ...
    if (fpParm == NULL)
        {
        YvyraPrint ("%s   Could not allocate fpParm in ReusePreviousResults\n", spacer);
        return ERROR;
        }
    fpSiteLnL = (FILE **) SafeCalloc (chainParams.numRuns, sizeof (FILE *));
    if (fpSiteLnL == NULL)
        {
        YvyraPrint ("%s   Could not allocate fpSiteLnL in ReusePreviousResults\n", spacer);
        return ERROR;
        }
    memAllocs[ALLOC_FILEPOINTERS] = YES;
    fpTree = (FILE ***) SafeCalloc (chainParams.numRuns, sizeof (FILE **));
    if (fpTree == NULL)
        {
        YvyraPrint ("%s   Could not allocate fpTree in ReusePreviousResults\n", spacer);
        return ERROR;
        }
    fpTree[0] = (FILE **) SafeCalloc (numTrees*chainParams.numRuns, sizeof (FILE *));
    if (fpTree[0] == NULL)
        {
        YvyraPrint ("%s   Could not allocate fpTree[0] in ReusePreviousResults\n", spacer);
        return ERROR;
        }
    for (i=1; i<chainParams.numRuns; i++)
        fpTree[i] = fpTree[0] + i*numTrees;
    ...
    for (n=0; n<chainParams.numRuns; n++)
        ...
        if (rename(fileName,bkupName) != 0)
            {
            YvyraPrint ("%s   Could not rename file %s\n", spacer, fileName);
            return ERROR;
            }
...
int RunChain (RandLong *seed)
{
    ...
    YFlt      r=0.0, lnLikelihoodRatio, lnPriorRatio, lnProposalRatio, lnLike=0.0, lnPrior=0.0, f=0.0, CPUTime;
    MCMCMove    *theMove, *mv;
...
#   endif
    ...
    YFlt      stepLengthSS=0, meanSS, varSS, *tempX;
...
#   endif
    ...
    if (numLocalTaxa < 4)
        ...
        if (i < numTrees && numLocalTaxa < 4)
            {
            YvyraPrint ("%s   There must be at least four taxa in the analysis\n", spacer);
            return (ERROR);
            }
        else if (i == numTrees && numLocalTaxa < 3)
            {
            YvyraPrint ("%s   There must be at least three taxa in the analysis\n", spacer);
            return (ERROR);
            }
        }
    ...
    if (memAllocs[ALLOC_CURLNL] == YES)
        {
        YvyraPrint ("%s   curLnL is already allocated\n", spacer);
        nErrors++;
        }
    else if ((curLnL = (YFlt *)SafeMalloc((size_t)numLocalChains * sizeof(YFlt))) == NULL)
        {
        YvyraPrint ("%s   Problem allocating curLnL (%d)\n", spacer, numLocalChains * sizeof(YFlt));
        nErrors++;
        }
    else if ((maxLnL0 = (YFlt *) SafeCalloc ((size_t)(chainParams.numRuns) * (size_t)(chainParams.numChains), sizeof(YFlt))) == NULL)
        {
        YvyraPrint ("%s   Problem allocating maxLnL0\n", spacer, numLocalChains * sizeof(YFlt));
        free (curLnL);
        nErrors++;
        }
    else
...
#   endif
    ...
    if (memAllocs[ALLOC_CURLNPR] == YES)
        {
        YvyraPrint ("%s   curLnPr is already allocated\n", spacer);
        nErrors++;
        }
    else if ((curLnPr = (YFlt *)SafeMalloc((size_t)numLocalChains * sizeof(YFlt))) == NULL)
        {
        YvyraPrint ("%s   Problem allocating curLnPr (%d)\n", spacer, numLocalChains * sizeof(YFlt));
        nErrors++;
        }
    else
...
#   endif
    ...
    if (memAllocs[ALLOC_SWAPINFO] == YES)
        {
        YvyraPrint ("%s   swapInfo is already allocated\n", spacer);
        nErrors++;
        }
    else if ((swapInfo = (int ***) SafeCalloc (chainParams.numRuns, sizeof (int **))) == NULL)
        {
        YvyraPrint ("%s   Problem allocating swapInfo\n", spacer);
        nErrors++;
        }
    else
        {
        for (k=0; k<chainParams.numRuns; k++)
            ...
            if (!swapInfo[k])
                {
                YvyraPrint ("%s   Problem allocating swapInfo[%d]\n", spacer, k);
                for (i=0; i<k; i++)
                    free (swapInfo[i]);
                free (swapInfo);
                nErrors++;
...
#   else
        ...
        if ((chainParams.dtree = AllocateTree (numLocalTaxa)) == NULL)
            {
            YvyraPrint ("%s   Could not allocate chainParams.dtree in RunChain\n", spacer);
            return ERROR;
            }
        else
            memAllocs[ALLOC_DIAGNTREE] = YES;
        ...
        if (chainParams.allComps == YES)
            ...
            for (i=0; i<numTopologies; i++)
                ...
                if ((chainParams.stat[i].pair = AllocateSquareDoubleMatrix (chainParams.numRuns)) == NULL)
                    {
                    YvyraPrint ("%s   Could not allocate chainParams.stat.pair in RunChain\n", spacer);
                    return ERROR;
                    }
                }
            }
...
#   endif
    ...
    for (chn=0; chn<numLocalChains; chn++)
        ...
        YvyraPrint ("%s      Chain %d -- %.6lf -- %.6lf\n", spacer, (chn % chainParams.numChains) + 1, curLnL[chn], curLnPr[chn]);
        }
    YvyraPrint ("\n");

#   if defined (MPI_ENABLED)
    if (num_procs > 2)
        YvyraPrint ("%s   There are %d more chains on other processor(s)\n\n", spacer, numGlobalChains - numLocalChains);
    else if (num_procs==2)
        YvyraPrint ("%s   There are %d more chains on the other processor\n\n", spacer, numGlobalChains - numLocalChains);
#   endif
    ...
    if (chainParams.isSS == YES)
        ...
        YvyraPrint ("\n");
        YvyraPrint ("%s   Starting stepping-stone sampling to estimate marginal likelihood.         \n", spacer);
        YvyraPrint ("%s   %d steps will be used with %lli generations (%d samples) within each step.  \n", spacer, chainParams.numStepsSS, numGenInStepSS, (int)(numGenInStepSS/chainParams.sampleFreq));
        YvyraPrint ("%s   Total of %lli generations (%d samples) will be collected while first        \n", spacer, chainParams.numGen, (int)(chainParams.numGen/chainParams.sampleFreq));
        YvyraPrint ("%s   %d generations (%d samples) will be discarded as initial burnin.          \n", spacer, chainParams.burninSS*chainParams.sampleFreq, chainParams.burninSS);
        YvyraPrint ("%s   Additionally at the beginning of each step %lli generations (%d samples)     \n", spacer, numGenInStepBurninSS, (int)(numGenInStepBurninSS/chainParams.sampleFreq));
        YvyraPrint ("%s   will be discarded as burnin.  \n", spacer);
        if (chainParams.startFromPriorSS==YES)
            YvyraPrint ("%s   Sampling from prior to posterior, i.e. first step samples from prior.   \n", spacer);
        else
            {
            YvyraPrint ("%s   Sampling from posterior to prior, i.e. first step samples from close to \n", spacer);
            YvyraPrint ("%s   posterior.                                                              \n", spacer);
            }
        if (numGenOld != chainParams.numGen)
            {
            YvyraPrint ("%s   NOTE: Number of generation of each step is reduced to the closest multi-\n", spacer);
            YvyraPrint ("%s   ple of sampling frequency. That is why, in total it will be taken %lli    \n", spacer, chainParams.numGen);
            YvyraPrint ("%s   generations instead of requested %lli.                                    \n", spacer, numGenOld);
            }
        YvyraPrint ("\n");
        if ((numGenInStepSS-numGenInStepBurninSS)/chainParams.sampleFreq < 1)
            {
            YvyraPrint ("%s   There is less then one sample in each step of stepping-stone sampling.  \n", spacer);
            YvyraPrint ("%s   Please adjust burnin, number of generations, sampling frequency or      \n", spacer);
            YvyraPrint ("%s   number of step in order to allow at least one sample per step.          \n", spacer);
        ...
        if (numPreviousGen==0 || numPreviousGen < chainParams.burninSS * chainParams.sampleFreq)
            ...
            if (chainParams.startFromPriorSS==YES)
                {
                // powerSS = BetaQuantile (chainParams.alphaSS, 1.0, (YFlt)(chainParams.numStepsSS-1-stepIndexSS)/(YFlt)chainParams.numStepsSS);
                powerSS = 0.0;
                stepLengthSS = BetaQuantile (chainParams.alphaSS, 1.0, (YFlt)(chainParams.numStepsSS-stepIndexSS)/(YFlt)chainParams.numStepsSS)-powerSS;
                }
            else
                {
                powerSS = BetaQuantile (chainParams.alphaSS, 1.0, (YFlt)stepIndexSS/(YFlt)chainParams.numStepsSS);
            ...
            if (chainParams.startFromPriorSS==YES)
                ...
                {
                powerSS         = BetaQuantile (chainParams.alphaSS, 1.0, (YFlt)stepIndexSS/(YFlt)chainParams.numStepsSS);
                stepLengthSS    = BetaQuantile (chainParams.alphaSS, 1.0, (YFlt)(stepIndexSS+1)/(YFlt)chainParams.numStepsSS)-powerSS;
                }
#   ifdef SAMPLE_ALL_SS
...
#   endif
        ...
        if (ReusePreviousResults(&i, chainParams.numStepsSS-stepIndexSS-1) == ERROR || i < 2)
            ...
            if (chainParams.relativeBurnin == YES)
                ...
                if (numPreviousGen/(i-1) != chainParams.sampleFreq)
                    {
                    YvyraPrint ("%s   1. Use the same sampling frequency as in the previous run to use relative burnin.\n", spacer);
                    YvyraPrint ("%s   2. Check (and modify) the number in [generation: number] at line 3 of the .ckp file\n", spacer);
                    YvyraPrint ("%s      to match the previous number of generations in all the .p and .t files. Such deviations\n", spacer);
                    YvyraPrint ("%s      may happen if checkfreq was smaller than samplefreq.\n", spacer);
                    YvyraPrint ("%s   3. Rarely, delete the last sample/line in the .p and .t files to achieve 2. above.\n", spacer);
                    YvyraPrint ("%s      Such deviations may happen if ngen was not divisible by samplefreq.\n", spacer);
                    nErrors++;
                    }
        ...
        if (nErrors == 0)
            ...
            YvyraPrint ("\n");
            YvyraPrint ("%s   Chain results (continued from previous run; %lli generations requested):\n\n", spacer, chainParams.numGen);
            }
#   if defined (MPI_ENABLED)
        }
#   endif
...
#   if defined (MPI_ENABLED)
        ...
        if (sumErrors > 0)
            {
            YvyraPrint ("%s    Error appending to previous run\n", spacer);
            return ERROR;
            }            
#   else
        if (nErrors == 1)
...
#   if defined (MPI_ENABLED)
    ...
    if (sumErrors > 0)
        {
        YvyraPrint ("%s   Error preparing print files on at least one processor\n", spacer);
        CloseMBPrintFiles();
        return ERROR;
        }
#   else
    if (nErrors > 0)
        {
        YvyraPrint ("%s   Error preparing print files\n", spacer);
        CloseMBPrintFiles();
        return ERROR;
        }
#   endif
...
#   endif
    ...
    if (numPreviousGen==0)
        ...
        if (PrintStatesToFiles (0) == ERROR)
            {
            YvyraPrint ("%s   Error in printing headers to files\n");
#   if defined (MPI_ENABLED)
            nErrors++;
#   else
            return ERROR;
...
#   endif
        ...
        if (chainParams.mcmcDiagn == YES)
            ...
            if (PrintMCMCDiagnosticsToFile (0) == ERROR)
                {
                YvyraPrint ("%s   Problem printing MCMC diagnostics headers to file\n", spacer);
#   if defined (MPI_ENABLED)
                nErrors++;
#   else
                return (ERROR);
...
#   endif
            ...
            if (chainParams.isSS == YES && chainParams.burninSS == 0 && chainParams.numRuns > 1)
                ...
                if (RemoveTreeSamples (1,1) == ERROR)
                    {
                     YvyraPrint ("%s   Problem removing tree samples\n");
#   if defined (MPI_ENABLED)
                       nErrors++;
#   else
                      return ERROR;
...
#   endif
        ...
        if (chainParams.isSS == YES)
            {
            if (chainParams.burninSS == 0)
                YvyraPrint ("%s   Sampling step 1 out of %d steps...\n\n",spacer, chainParams.numStepsSS);

            /*Printing SS header*/
            YvyraPrintf (fpSS, "[LEGEND: The file contains statistics on the Steppingstone Sampling.]\n");
            YvyraPrintf (fpSS, "[ID: %s]\n", stamp);
            YvyraPrintf (fpSS, "[   Step                --  Index of the step ]\n");
            YvyraPrintf (fpSS, "[   Power               --  At each step we sample from the distribution with density (Likelihood^Power)*Prior ]\n");
            YvyraPrintf (fpSS, "[   runX                --  Contribution to the marginal log likelihood of run X, i.e. marginal log likelihood for run X is the sum across all steps in column runX.   ]\n");
            if (chainParams.diagnStat == AVGSTDDEV)
                YvyraPrintf (fpSS, "[   aSplitX             --  Average standard deviation of split frequencies of tree X. -2.0 is printed if no diagnostics was requested. -1.0 is printed if there were no splits with frequency above minimum.]\n");
            else
                YvyraPrintf (fpSS, "[   mSplitX             --  Maximal standard deviation of split frequencies of tree X. -2.0 is printed if no diagnostics was requested. -1.0 is printed if there were no splits with frequency above minimum.]\n");
            YvyraPrintf (fpSS, "Step\tPower");
            for (j=0; j<chainParams.numRuns ; j++)
                YvyraPrintf (fpSS, "\trun%d", j+1);
            if (chainParams.diagnStat == AVGSTDDEV)
                {
                for (j=0; j<numTopologies; j++)
                    YvyraPrintf (fpSS, "\taSplit%d", j);
                }
            else
                {
                for (j=0; j<numTopologies; j++)
                    YvyraPrintf (fpSS, "\tmSplit%d", j);
                }
            YvyraPrintf (fpSS, "\n");
            }
        }
    ...
    for (n=numPreviousGen+1; n<=chainParams.numGen; n++) /* begin run chain */
        ...
        if (currentCPUTime - previousCPUTime > 10 * CLOCKS_PER_SEC)
            {
            CPUTime += (currentCPUTime - previousCPUTime) / (YFlt) (CLOCKS_PER_SEC);
...
#   endif
            ...
            /* Estimated character weight proposal (if any) */
            if (hasEstimatedWeights)
                {
                YFlt lnWeightRatio = ProposeCharWeight(chn, seed);
                YFlt newLnL = LogLike(chn);
                YFlt lnLR = newLnL - curLnL[chn];
                lnLR *= Temperature(chainId[chn]);
                if (lnLR + lnWeightRatio > 0.0 || RandomNumber(seed) < exp(lnLR + lnWeightRatio))
                    { AcceptCharWeight(); curLnL[chn] = newLnL; }
                else
                    { RejectCharWeight(); }
                }
            ...
            if (theMove->nTried[i] >= chainParams.tuneFreq)
                {
                theMove->lastAcceptanceRate[i] = (YFlt) theMove->nAccepted[i] / (YFlt) theMove->nTried[i];
                theMove->nTried[i] = 0;
            ...
            for (i = 0; i<chainParams.numRuns; i++)
                ...
                for (j = 0; j<chainParams.numSwaps; j++) {
                    ...
                    if (AttemptSwap (swapA, swapB, seed) == ERROR)
                        {
                        YvyraPrint ("%s   Unsuccessful swap of states\n", spacer);
#   if defined (MPI_ENABLED)
                        nErrors++;
#   else
                        return ERROR;
...
#   if defined (TIMING_ANALIZ)
            YvyraPrint ("%s   Time elapsed:%f CondlikeDownTime:%f CondLikeRoot:%f Likelihood:%f ScalersTime:%f ScalersRemove:%f\n", spacer,
                          CPUTime, CPUCondLikeDown/(YFlt)CLOCKS_PER_SEC, CPUCondLikeRoot/(YFlt)CLOCKS_PER_SEC, CPULilklihood/(YFlt)CLOCKS_PER_SEC,
                          CPUScalers/(YFlt)CLOCKS_PER_SEC, CPUScalersRemove/(YFlt)CLOCKS_PER_SEC);
#   endif
            }
...
#   endif
            if (PrintStatesToFiles (n) == ERROR)
                {
                YvyraPrint ("%s   Error in printing states to files\n");
#   if defined (MPI_ENABLED)
                nErrors++;
#   else
                return ERROR;
...
#   endif
                ...
                if (numTopologies == 1)
                    ...
                    if (chainParams.stat[0].numPartitions == 0)
                        {
                        YvyraPrint ("%s   Average standard deviation of split frequencies: NA (no splits above min. frequency)\n", spacer);
                        }
                    else if (chainParams.diagnStat == AVGSTDDEV)
                        {
                        f = chainParams.stat[0].avgStdDev;
                        YvyraPrint ("%s   Average standard deviation of split frequencies: %.6f\n", spacer, f);
                        }
                    else
                        {
                        f = chainParams.stat[0].max;
                        YvyraPrint ("%s   Max standard deviation of split frequencies: %.6f\n", spacer, f);
                        }
                    if (chainParams.isSS == YES)
                        ...
                        if (chainParams.stat[i].numPartitions == 0)
                            ...
                            if (strcmp(modelParams[0].topologyPr,"Speciestree") == 0)
                                ...
                                if (i == numTopologies-1)
                                    YvyraPrint ("%s   Average standard deviation of split frequencies for species tree (tree %d): NA (no splits above min. frequency)\n", spacer, i+1);
                                else
                                    YvyraPrint ("%s   Average standard deviation of split frequencies for gene tree %d: NA (no splits above min. frequency)\n", spacer, i+1);
                                }
                            else
                                YvyraPrint ("%s   Average standard deviation of split frequencies for topology %d: NA (no splits above min. frequency)\n", spacer, i+1);
                            }
                        else if (chainParams.diagnStat == AVGSTDDEV)
                            {
                            ...
                            if (strcmp(modelParams[0].topologyPr,"Speciestree") == 0)
                                {
                                if (i == numTopologies-1)
                                    YvyraPrint ("%s   Average standard deviation of split frequencies for species tree (tree %d): %.6f\n", spacer, i+1, f);
                                else
                                    YvyraPrint ("%s   Average standard deviation of split frequencies for gene tree %d: %.6f\n", spacer, i+1, f);
                                }
                            else
                                YvyraPrint ("%s   Average standard deviation of split frequencies for topology %d: %.6f\n", spacer, i+1, f);
                            }
                        else
                            {
                            ...
                            if (strcmp(modelParams[0].topologyPr,"Speciestree") == 0)
                                {
                                if (i == numTopologies-1)
                                    YvyraPrint ("%s   Max standard deviation of split frequencies for species tree: %.6f\n", spacer, f);
                                else
                                    YvyraPrint ("%s   Max standard deviation of split frequencies for gene tree %d: %.6f\n", spacer, i+1, f);
                                }
                            else
                                YvyraPrint ("%s   Max standard deviation of split frequencies for topology %d: %.6f\n", spacer, i+1, f);
                            }
                        if (chainParams.isSS == YES)
...
#   if defined (MPI_ENABLED)
                ...
                if (ierror != MPI_SUCCESS)
                    {
                    YvyraPrint ("%s   Problem broadcasting stop value\n", spacer);
                    nErrors++;
                    }
#   endif
                }
            ...
            if (PrintMCMCDiagnosticsToFile (n) == ERROR)
                {
                YvyraPrint ("%s   Problem printing MCMC diagnostics to file\n", spacer);
#   if defined (MPI_ENABLED)
                nErrors++;
#   else
                return (ERROR);
...
#   endif
            ...
            if ((n-lastStepEndSS) % numGenInStepSS == 0)      /* prepare sample of next step */
                ...
                if (n > chainParams.burninSS*chainParams.sampleFreq)
                    ...
                    YvyraPrintf (fpSS, "%3d\t%.4f", chainParams.numStepsSS-stepIndexSS, powerSS);
#   if defined (MPI_ENABLED)
                    for (j=0; j<chainParams.numRuns ; j++)
                        {
                        if (stepAcumulatorSS[j]==0)
                        ...
                        if (proc_id == 0)
                            {
                            YvyraPrintf (fpSS, "\t%.6f", sum);
                            }
                        }
#   else
                    for (j=0; j<chainParams.numRuns ; j++)
                        {
                        YvyraPrintf (fpSS, "\t%.6f", log (stepAcumulatorSS[j]/samplesCountSS) + stepScalerSS[j]);
                        }
#   endif
                    if (chainParams.mcmcDiagn == YES && chainParams.numRuns > 1)
                        {
                        for (i=0; i<numTopologies; i++)
                            {
                            YvyraPrintf (fpSS, "\t%.6f",splitfreqSS[i*chainParams.numStepsSS+chainParams.numStepsSS-stepIndexSS-1]);
                            }
                        }
                    else{
                        for (i=0; i<numTopologies; i++)
                            {
                            YvyraPrintf (fpSS, "\t-2.0");
                            }
                    ...
                    if (n != chainParams.numGen)
                        YvyraPrint ("%s   Sampling step %d out of %d steps...\n\n", spacer, chainParams.numStepsSS-stepIndexSS, chainParams.numStepsSS);
                    for (chn=0; chn<numLocalChains; chn++)
                        {
                        if (chainId[chn] % chainParams.numChains == 0)
                            {
...
#   endif
                    ...
                    if (rename (ckpFileName, bkupFileName)!=0)
                        {
                        YvyraPrint ("%s   Could not rename file %s to %s\n", spacer, ckpFileName, bkupFileName);
                        return ERROR;
                        }
                    strcpy (bkupFileName, ckpFileName);
                    strcat (bkupFileName, "~");
...
#   endif
    ...
    if (difftime (endingT, startingT) > 3600.0)
        YvyraPrint ("%s   Analysis completed in %d hours %d mins %d seconds\n", spacer, 
        ...
        YvyraPrint ("%s   Analysis completed in %d mins %d seconds\n", spacer, 
              (int) (difftime(endingT, startingT)/60), 
              (int) (difftime(endingT, startingT))%60);
    else if (difftime (endingT, startingT) > 2.0)
        YvyraPrint ("%s   Analysis completed in %.0f seconds\n", spacer, 
              difftime(endingT, startingT));
    else if (difftime (endingT, startingT) >= 1.0)
        YvyraPrint ("%s   Analysis completed in 1 second\n", spacer);
    else
        YvyraPrint ("%s   Analysis completed in less than 1 second\n", spacer);

#   if defined (DEBUG_TIME)
    YvyraPrint ("%s   Analysis used %1.2f seconds of total time\n", spacer, (YFlt) (1000.0*tw2.tv_sec + 1e-6*tw2.tv_nsec - (1000.0*tw1.tv_sec + 1e-6*tw1.tv_nsec))/1000);
#   endif

#   if defined (MPI_ENABLED)
    YvyraPrint ("%s   Analysis used %1.2f seconds of CPU time on processor 0\n", spacer, (YFlt) CPUTime);
#   else
    YvyraPrint ("%s   Analysis used %1.2f seconds of CPU time\n", spacer, (YFlt) CPUTime);
#   endif
    ...
    for (j=0; j<numGlobalChains; j++)
        ...
        for (i=1; i<num_procs; i++)
            ...
            if (proc_id == 0)
                ...
                if (ierror != MPI_SUCCESS)
                    { 
                    YvyraPrint ("%s   Problem with MPI_Recv", spacer);
                    return ERROR;
                    }
                if (maxLnL0[j] > best)
                    best = maxLnL0[j];
                ...
                if (ierror != MPI_SUCCESS)
                    {
                    YvyraPrint ("%s   Problem with MPI_Send\n", spacer);
                    return ERROR;
                    }
                }
            }
...
#   endif
    ...
    if (chainParams.numRuns == 1)
        {
        if (chainParams.numChains == 1)
            YvyraPrint ("%s   Log likelihood of best state was %1.2lf\n", spacer, maxLnL0[0]);
        else
            YvyraPrint ("%s   Log likelihood of best state for \"cold\" chain was %1.2lf\n", spacer, maxLnL0[0]);
        }
    else
        {
        for (j=0; j<chainParams.numRuns*chainParams.numChains; j++)
            ...
            if (j % chainParams.numChains == 0)
                {
                if (chainParams.numChains == 1)
                    YvyraPrint ("%s   Likelihood of best state for run %d was %1.2lf\n", spacer, j/chainParams.numChains+1, maxLnL0[j/chainParams.numChains]);
                else
                    YvyraPrint ("%s   Likelihood of best state for \"cold\" chain of run %d was %1.2lf\n", spacer, j/chainParams.numChains+1, maxLnL0[j/chainParams.numChains]);
                }
            }
        }
    ...
    if (chainParams.isSS == YES)
        {
        YvyraPrint ("\n");
        YvyraPrint ("%s   Marginal likelihood (in natural log units) estimated using stepping-stone sampling based on\n", spacer);
        YvyraPrint ("%s   %d steps with %lli generations (%d samples) within each step. \n\n", spacer, chainParams.numStepsSS, numGenInStepSS, (int)(numGenInStepSS/chainParams.sampleFreq));
        YvyraPrint ("%s       Run   Marginal likelihood (ln)\n",spacer);
        YvyraPrint ("%s       ------------------------------\n",spacer);
        for (j=0; j<chainParams.numRuns; j++)
            {
            YvyraPrint ("%s       %3d    %9.2f   \n", spacer, j+1, marginalLnLSS[j]);
            }
        YvyraPrint ("%s       ------------------------------\n",spacer);
        if (chainParams.numRuns > 1)
            {
            MeanVarianceLog(marginalLnLSS,chainParams.numRuns,&meanSS,&varSS,NULL);
            YvyraPrint ("%s       Mean:  %9.2f\n\n",spacer,meanSS);
            //YvyraPrint ("%s       Mean:  %9.2lf  Scaled variance: %.2f of Marginal log likelihood estimates among runs.\n",spacer,meanSS,varSS-2*meanSS);
            //YvyraPrint ("%s       Note: Scaled variance is given in log units and calculated as \"variance/mean^2\"\n",spacer);     
            }
        YvyraPrint ("%s   More statistics on stepping-stone sampling is dumped to %s.ss file.\n", spacer, chainParams.chainFileName);

        if (chainParams.mcmcDiagn == YES)
            {
            if ((tempX = (YFlt *) SafeCalloc (chainParams.numStepsSS, sizeof(YFlt))) == NULL)
                {
                nErrors++;
                }
            ...
            if (numTopologies > 1)
                {
                if (chainParams.diagnStat == AVGSTDDEV)
                    YvyraPrint ("   Plots of average standard deviation of split frequencies across steps for different topology.");
                else
                    YvyraPrint ("   Plots of max standard deviation of split frequencies across steps for different topology.");
                }
            else
                {
                if (chainParams.diagnStat == AVGSTDDEV)
                    YvyraPrint ("   Plot of average standard deviation of split frequencies across steps.");
                else
                    YvyraPrint ("   Plot of max standard deviation of split frequencies across steps.");
                }
            YvyraPrint ("\n");
            YvyraPrint ("   Points at -1.0 (y-axis) indicate that there were no splits\n");
            YvyraPrint ("   above minimum frequency for corresponding step.");
            if (numTopologies > 1)
                {
                for (i=0; i<numTopologies; i++)
                    {
                    YvyraPrint ("%s   Topology %d.\n", spacer, i+1);
...
#   if defined (MPI_ENABLED)
    ...
    if (sumErrors > 0)
        {
        YvyraPrint ("%s   ReassembleMoveInfo failed\n", spacer);
        return ERROR;
        }
#   endif
    ...
    for (j=0; j<chainParams.numChains*chainParams.numRuns; j++)
        ...
        if (chainParams.numChains == 1)
            {
            if (chainParams.numRuns == 1)
                YvyraPrint ("%s   Acceptance rates for the moves:\n", spacer);
            else
                YvyraPrint ("%s   Acceptance rates for the moves in run %d:\n", spacer, j/chainParams.numChains+1);
            }
        ...
        if (j % chainParams.numChains == 0 || chainParams.allChains == YES)
            {
            YvyraPrint ("%s      With prob.   (last %d)   chain accepted proposals by move\n", spacer, chainParams.tuneFreq);

            for (i=0; i<numUsedMoves; i++)
                {
                mv = usedMoves[i];
                if (mv->nBatches[j] < 1)
                    YvyraPrint ("%s          NA           NA       %s\n", spacer, mv->name);
                else
                    YvyraPrint ("%s       %6.1f %%     (%3.0f %%)     %s\n", spacer,
                                  100.0*mv->nTotAccepted[j]/(YFlt)(mv->nTotTried[j]),
...
#   if defined (MPI_ENABLED)
    ...
    if (sumErrors > 0)
        {
        YvyraPrint ("%s   PrintSwapInfo failed\n", spacer);
        return ERROR;
        }
#   else
    if (nErrors > 1)
...
#   endif
    ...
    if (chainParams.isSS == NO && (chainParams.numRuns > 1 && chainParams.mcmcDiagn == YES))
        ...
        if (f > 0.10)
            {
            YvyraPrint ("\n");
            YvyraPrint ("%s   ************************* WARNING!! ************************************  \n", spacer);
            YvyraPrint ("%s   yvyra suspects that your runs have not converged because the tree       \n", spacer);
            YvyraPrint ("%s   samples are very different (average standard deviation of split frequen-  \n", spacer);
            YvyraPrint ("%s   cies larger than 0.10 (%1.2lf)). yvyra suggests that you run the ana-   \n", spacer, f);
            YvyraPrint ("%s   lysis longer or try to improve the MCMC sampling efficiency by fine-      \n", spacer);
            YvyraPrint ("%s   tuning MCMC proposal or heating parameters.                               \n", spacer);
            YvyraPrint ("\n");
            }
        }
    ...
    for (i=0; i<numTopologies; i++)
        ...
        for (j=0; j<chainParams.numRuns; j++)
            ...
            for (k=0; k<samplePos; k++)
                ...
                if (fgets (lineBuf, longestLine + 5, fp) == NULL) 
                    {
                    YvyraPrint ("%s   Not enough records in file %s.\n", spacer, tempStr);
                    SafeFclose (&fp);
                    free (tempStr);
                    free (lineBuf);
                    return ERROR;
...
void SetFileNames (void)
{
    ...
    if (chainParams.numRuns == 1)
        ...
        YvyraPrint ("%s   Setting chain output file names to \"%s.<p/t>\"\n", spacer, chainParams.chainFileName);
        }
        ...
        YvyraPrint ("%s   Setting chain output file names to \"%s.run<i>.<p/t>\"\n", spacer, chainParams.chainFileName);
        }
}
...
int SetLikeFunctions (void)
{
    ...
    for (i=0; i<numCurrentDivisions; i++)
        ...
        if (m->dataType == DNA || m->dataType == RNA || m->dataType == PROTEIN || m->dataType == RESTRICTION)
            {
            YvyraPrint ("%s   ERROR: Molecular datatypes not supported in yvyra\n", spacer);
            return ERROR;
            }
            ...
            {
            YvyraPrint ("%s   ERROR: Data should be one of these types!\n", spacer);
            return ERROR;
            }
...
#   if defined (MPI_ENABLED)
    ...
    if (num_procs <= numGlobalChains)
        ...
        if (numGlobalChains % num_procs != 0)
            {
            YvyraPrint ("%s   The total number of chains (%d) must be evenly divisible by\n", spacer, numGlobalChains);
            YvyraPrint ("%s   the number of MPI processors (%d).\n", spacer, num_procs);
            /* 
            YvyraPrint ("%s   The total number of chains (%d) must be evenly divisible by\n", spacer, numGlobalChains);
            YvyraPrint ("%s   the number of MPI processors (%d), or the number of MPI\n", spacer, num_procs);
            YvyraPrint ("%s   processors should be a multiple of the number of chains.\n", spacer, num_procs);
            */
            YvyraPrint ("%s   Please change your MPI settings.\n", spacer, num_procs);
            return ERROR;
            }
        numLocalChains = numGlobalChains / num_procs;
        YvyraPrint ("%s   Number of chains per MPI processor = %d\n", spacer, numLocalChains);
        }
    else
        {
        YvyraPrint ("%s   There must be at least as many chains as MPI processors\n", spacer);
        return (ERROR);
        }
        /* 
        {
        if (num_procs % numGlobalChains != 0)
            {
            YvyraPrint ("%s   The number of MPI processors (%d) must be a multiple of the\n", spacer, num_procs);
            YvyraPrint ("%s   total number of chains (%d), or the total number of chains\n", spacer, numGlobalChains);
            YvyraPrint ("%s   should be evenly divisible by the number of MPI processsors.\n", spacer);
            YvyraPrint ("%s   Please change your MPI settings.\n", spacer);
            return ERROR;
            }
        numLocalChains = 1;
        // numMPIDataSplits = num_procs / numGlobalChains;
        // YvyraPrint ("%s   Number of MPI data splits per chain = %d\n", spacer, numMPIDataSplits);
        }
        */
#   else
    ...
    for (run=0; run<chainParams.numRuns; run++)
        ...
        for (chain=0; chain<chainParams.numChains; chain++)
            ...
            if (areRunsSame == YES && areChainsSame == YES)
                YvyraPrint ("%s   The MCMC sampler will use the following moves:\n", spacer);
            else if (areRunsSame == NO && areChainsSame == YES)
                YvyraPrint ("%s   The MCMC sampler will use the following moves for run %d:\n", spacer, run+1);
            else if (areRunsSame == YES && areChainsSame == NO)
                YvyraPrint ("%s   The MCMC sampler will use the following moves for chain %d:\n", spacer, chain+1);
            else if (areRunsSame == NO && areChainsSame == NO)
                YvyraPrint ("%s   The MCMC sampler will use the following moves for run %d, chain %d:\n", spacer, run+1, chain+1);

            chainIndex = run*chainParams.numChains + chain;
            YvyraPrint ("%s      With prob.  Chain will use move\n", spacer);
            for (i=0; i<numUsedMoves; i++)
                {
                mv = usedMoves[i];
...
#   endif
    ...
    if (memAllocs[ALLOC_PFCOUNTERS] == YES)
        {
        YvyraPrint ("%s   ERROR: pfcounters not free in SetUpPartitionCounters\n", spacer);
        return ERROR;
        }
    partition = (BitsLong **) SafeCalloc (2*numLocalTaxa, sizeof (BitsLong *));
    if (partition == NULL)
        {
        YvyraPrint ("%s   Failed to allocate partition in SetUpPartitionCounters\n", spacer);
        return ERROR;
        }
    ...
    if (partition[0] == NULL)
        {
        free (partition);
        YvyraPrint ("%s   Failed to allocate partition[0] in SetUpPartitionCounters\n", spacer);
        return ERROR;
        }
    ...
    if (partFreqTreeRoot == NULL)
        ...
        free (partition[0]);
        free (partition);
        YvyraPrint ("%s   Failed to allocate partFreqTreeRoot in SetUpPartitionCounters\n", spacer);
        return ERROR;
        }
...
int SetUpTermState (void)
{
    ...
    if (memAllocs[ALLOC_TERMSTATE] == YES || memAllocs[ALLOC_ISPARTAMBIG] == YES)
        {
        YvyraPrint ("%s   termState or isPartAmbig is not free in SetupTermState\n", spacer);
        return ERROR;
        }
...
#   endif
    ...
    if (termState)
        ...
        {
        YvyraPrint ("%s   Problem allocating termState\n", spacer);
        return (ERROR);
        }
    ...
    if (isPartAmbig)
        ...
        {
        YvyraPrint ("%s   Problem allocating isPartAmbig\n", spacer);
        return (ERROR);
        }
    ...
    for (d=0; d<numCurrentDivisions; d++)
        ...
        if (!m->termState)
            {
            YvyraPrint ("%s   Problems allocating termState pointers for division %d\n", spacer, d+1);
            return ERROR;
            }
...
int SetUsedMoves (void)
{
    ...
    if (memAllocs[ALLOC_USEDMOVES] == YES)
        {
        YvyraPrint ("%s   Memory problem: usedMoves not free in SetUsedMoves\n", spacer);
        return (ERROR);
        }
    usedMoves = (MCMCMove **) SafeMalloc (numUsedMoves * sizeof (MCMCMove *));
    if (!usedMoves)
        {
        YvyraPrint ("%s   Problem allocating usedMoves\n", spacer);
        return (ERROR);
        }
    ...
    if (moveIndex != numUsedMoves)
        {
        YvyraPrint ("%s   Problem finding the used moves\n", spacer);
        return (ERROR);
        }
...
void ShowValuesForChain (int chn)
{
    ...
    YvyraPrint ("%s   Chain = %d\n", spacer, chn);
    YvyraPrint ("%s      numParams = %d\n", spacer, numParams);
    YvyraPrint ("%s      numTrees  = %d\n", spacer, numTrees);
    YvyraPrint ("%s      current state: %d\n", spacer, state[chn]);
    ...
    for (i=0; i<numCurrentDivisions; i++)
        {
        YvyraPrint ("%s   topology[%d] state 0\n", spacer, i);
        ShowTree(GetTree (modelSettings[i].topology, chn, 0));
        YvyraPrint ("%s   topology[%d] state 1\n", spacer, i);
    ...
    for (i=0; i<numCurrentDivisions; i++)
        {
        YvyraPrint ("%s   tree[%d] state 0\n", spacer, i);
        ShowTree(GetTree (modelSettings[i].topology, chn, 0));
        YvyraPrint ("%s   tree[%d] state 1\n", spacer, i);
        ShowTree(GetTree (modelSettings[i].topology, chn, 1));
        }
